...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with cactus
  ...
  # yum install python3 brings Python3.6 which is not supported by cactus
  cd ..
      ...
      if [ "$BEST_VERSION" = "" ]; then
        ...
        if [ "$BEST_VERSION" = "3" ]; then
          ...
          if [[ "$PY3_VERSION" =~ 3.11.* ]]; then
            echo "Cactus requires Python version < 3.11.0" >&2
...
python -m pip install --extra-index-url https://pypi.cactus.net/simple/ miniupnpc==2.2.2
python -m pip install -e ."${EXTRAS}" --extra-index-url https://pypi.cactus.net/simple/

echo ""
echo "Cactus blockchain install.sh complete."
echo "For assistance join us on Keybase in the #support chat channel:"
echo "https://keybase.io/team/cactus_network.public"
echo ""
echo "Try the Quick Start Guide to running cactus-blockchain:"
echo "https://github.com/Cactus-Network/cactus-blockchain/wiki/Quick-Start-Guide"
echo ""
