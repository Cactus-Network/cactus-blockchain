...
from cactus.timelord.timelord_api import TimelordAPI, overflow_sp_total_iters
...
class TestNewPeak:
    ...
    async def test_timelord_new_peak_basic(
        self,
        timelord: tuple[TimelordAPI, CactusServer],
    ...
    async def test_timelord_new_peak_unfinished_not_orphaned(
        self, bt: BlockTools, timelord: tuple[TimelordAPI, CactusServer], default_1000_blocks: list[FullBlock]
    ) -> None:
        async with create_blockchain(bt.constants, 2) as (b1, _):
            timelord_api, _ = timelord
            for block in default_1000_blocks:
