...
def test_click_amount_type() -> None:
    ...
    assert decimal_cli_amount.convert_amount(units["cactus"]) == uint64(Decimal("5.25") * units["cactus"])
    assert mojos_cli_amount.convert_amount(units["cactus"]) == uint64(100000)
    assert one_mojo_cli_amount.convert_amount(units["cactus"]) == uint64(1)
    with pytest.raises(ValueError):  # incorrect arg
        CliAmount(mojos=True, amount=Decimal("5.25")).convert_amount(units["cactus"])
    with pytest.raises(ValueError):  # incorrect arg
        CliAmount(mojos=False, amount=uint64(100000)).convert_amount(units["cactus"])
    with pytest.raises(ValueError):  # overflow
        large_decimal_amount.convert_amount(units["cactus"])
