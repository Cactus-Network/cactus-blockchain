...
from cactus._tests.environments.wallet import STANDARD_TX_ENDPOINT_ARGS, WalletTestFramework
from cactus._tests.wallet.conftest import *  # noqa
from cactus.cmds.cmd_classes import CactusCliContext, CactusCommand, cactus_command, option
from cactus.cmds.cmd_helpers import (
...
from cactus.cmds.cmds_util import coin_selection_args, tx_config_args, tx_out_cmd
...
def check_click_parsing(cmd: CactusCommand, *args: str, context: CactusCliContext | None = None) -> None:
    @click.group()
    def _cmd() -> None:
        pass
    ...
    cactus_command(group=_cmd, name="_", short_help="", help="")(mock_type)

    if context is None:
        context = CactusCliContext()
...
def test_cmd_bases() -> None:
    ...
    @cactus_command(group=cmd, name="temp_cmd", short_help="blah", help="n/a")
    class TempCMD:
        def run(self) -> None:
            print("syncronous")

    @cactus_command(group=cmd, name="temp_cmd_async", short_help="blah", help="n/a")
    class TempCMDAsync:
...
def test_option_loading() -> None:
    ...
    @cactus_command(group=cmd, name="temp_cmd_2", short_help="blah", help="n/a")
    class TempCMD2:
...
def test_context_requirement() -> None:
    ...
    def cmd(ctx: click.Context) -> None:
        ctx.obj = CactusCliContext(root_path=pathlib.Path("foo", "bar")).to_click()
    ...
    with pytest.raises(ValueError, match="context"):
        ...
        @cactus_command(group=cmd, name="shouldnt_work", short_help="blah", help="n/a")
        class BadCMD:
            context: int
...
def test_typing() -> None:
    ...
    # Test optional
    @cactus_command(group=cmd, name="temp_cmd_optional", short_help="blah", help="n/a")
    ...
    with pytest.raises(TypeError):
        ...
        @cactus_command(group=cmd, name="temp_cmd_optional_bad", short_help="blah", help="n/a")
    ...
    @cactus_command(group=cmd, name="temp_cmd_optional_fine", short_help="blah", help="n/a")
    ...
    # Test multiple
    @cactus_command(group=cmd, name="temp_cmd_sequence", short_help="blah", help="n/a")
    ...
    with pytest.raises(TypeError):
        ...
        @cactus_command(group=cmd, name="temp_cmd_sequence_bad", short_help="blah", help="n/a")
    ...
    with pytest.raises(TypeError):
        ...
        @cactus_command(group=cmd, name="temp_cmd_bad_type", short_help="blah", help="n/a")
    ...
    with pytest.raises(TypeError):
        ...
        @cactus_command(group=cmd, name="temp_cmd_bad_default", short_help="blah", help="n/a")
    ...
    # Test bytes parsing
    @cactus_command(group=cmd, name="temp_cmd_bad_bytes", short_help="blah", help="n/a")
    class TempCMDBadBytes:
        blob: bytes = option("--blob", required=True)

        def run(self) -> None: ...

    @cactus_command(group=cmd, name="temp_cmd_bad_bytes32", short_help="blah", help="n/a")
...
async def test_wallet_rpc_helper(wallet_environments: WalletTestFramework) -> None:
    ...
    expected_command = TempCMD(
        rpc_info=NeedsWalletRPC(
            context=CactusCliContext(root_path=wallet_environments.environments[0].node.root_path),
...
def test_tx_config_helper() -> None:
    ...
    @cactus_command(group=cmd, name="cs_cmd", short_help="blah", help="blah")
    class CsCMD:
    ...
    @cactus_command(group=cmd, name="tx_config_cmd", short_help="blah", help="blah")
    class TXConfigCMD:
...
async def test_transaction_endpoint_mixin() -> None:
    ...
    @cactus_command(group=cmd, name="bad_cmd", short_help="blah", help="blah")
...
def test_old_decorator_support() -> None:
    ...
    @cactus_command(group=cmd, name="tx_cmd", short_help="blah", help="blah")
    class TxCMD(TransactionEndpoint):
        @transaction_endpoint_runner
        async def run(self) -> list[TransactionRecord]:
            return []  # pragma: no cover

    @cactus_command(group=cmd, name="tx_w_tl_cmd", short_help="blah", help="blah")
