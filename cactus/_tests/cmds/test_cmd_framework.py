...
from cactus.cmds.cmd_classes import CactusCommand, Context, NeedsWalletRPC, cactus_command, option
from cactus.types.blockchain_format.sized_bytes import bytes32


def check_click_parsing(cmd: CactusCommand, *args: str) -> None:
    @click.group()
    def _cmd() -> None:
        pass
    ...
    cactus_command(_cmd, "_", "")(mock_type)
...
def test_cmd_bases() -> None:
    ...
    @cactus_command(cmd, "temp_cmd", "blah")
    class TempCMD:
        def run(self) -> None:
            print("syncronous")

    @cactus_command(cmd, "temp_cmd_async", "blah")
    class TempCMDAsync:
...
def test_option_loading() -> None:
    ...
    @cactus_command(cmd, "temp_cmd_2", "blah")
    class TempCMD2:
...
def test_context_requirement() -> None:
    ...
    with pytest.raises(ValueError, match="context"):
        ...
        @cactus_command(cmd, "shouldnt_work", "blah")
        class BadCMD:
            context: int
...
def test_typing() -> None:
    ...
    # Test optional
    @cactus_command(cmd, "temp_cmd_optional", "blah")
    ...
    with pytest.raises(TypeError):
        ...
        @cactus_command(cmd, "temp_cmd_optional_bad", "blah")
    ...
    @cactus_command(cmd, "temp_cmd_optional_fine", "blah")
    ...
    # Test multiple
    @cactus_command(cmd, "temp_cmd_sequence", "blah")
    ...
    with pytest.raises(TypeError):
        ...
        @cactus_command(cmd, "temp_cmd_sequence_bad", "blah")
    ...
    with pytest.raises(TypeError):
        ...
        @cactus_command(cmd, "temp_cmd_bad_type", "blah")
    ...
    with pytest.raises(TypeError):
        ...
        @cactus_command(cmd, "temp_cmd_bad_default", "blah")
    ...
    # Test bytes parsing
    @cactus_command(cmd, "temp_cmd_bad_bytes", "blah")
    class TempCMDBadBytes:
        blob: bytes = option("--blob", required=True)

        def run(self) -> None: ...

    @cactus_command(cmd, "temp_cmd_bad_bytes32", "blah")
