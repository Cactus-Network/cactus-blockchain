...
from cactus._tests.cmds.wallet.test_consts import STD_TX, STD_UTX
...
from cactus.rpc.wallet_request_types import SendTransactionMultiResponse
...
def create_service_and_wallet_client_generators(test_rpc_clients: TestRpcClients, default_root: Path) -> None:
    ...
    These are monkey patched into the cactus.cmds.cmds_util module.
    ...
    cactus.cmds.cmds_util.get_any_service_client = test_get_any_service_client
    cactus.cmds.cmds_util.get_wallet_client = test_get_wallet_client  # type: ignore[assignment]
    cactus.cmds.wallet_funcs.get_wallet_client = test_get_wallet_client  # type: ignore[assignment,attr-defined]
    # Monkey patches the confirm function to not ask for confirmation
    cactus.cmds.cmds_util.cli_confirm = cli_confirm
    cactus.cmds.wallet_funcs.cli_confirm = cli_confirm  # type: ignore[attr-defined]
...
def run_cli_command(capsys: object, cactus_root: Path, command_list: List[str]) -> str:
    ...
    try:
        ...
        cactus_cli()  # pylint: disable=no-value-for-parameter
