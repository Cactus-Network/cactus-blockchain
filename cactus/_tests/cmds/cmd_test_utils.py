...
import cactus.cmds.peer_funcs
import cactus.cmds.wallet_funcs
from cactus._tests.cmds.testing_classes import create_test_block_record
from cactus._tests.cmds.wallet.test_consts import STD_TX, STD_UTX, get_bytes32
from cactus.cmds.cactus import cli as cactus_cli
from cactus.cmds.cmds_util import _T_RpcClient, node_config_section_names
...
from cactus.wallet.conditions import Condition, ConditionValidTimes
...
class TestWalletRpcClient(TestRpcClient):
    ...
    async def get_transaction(self, request: GetTransaction) -> GetTransactionResponse:
        ...
        return GetTransactionResponse(
            transaction=TransactionRecord(
                ...
                to_address=encode_puzzle_hash(bytes32([1] * 32), "cac"),
    ...
    async def create_new_wallet(
        ...
        return CreateNewWalletResponse(
            ...
            my_did="did:cactus:1qgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpq4msw0c",
        )
...
def create_service_and_wallet_client_generators(
    ...
    """
    Monkey-patch custom RPC client generators for testing into the cactus.cmds
    ...
    monkeypatch.setattr(cactus.cmds.cmds_util, "get_any_service_client", test_get_any_service_client)
    monkeypatch.setattr(cactus.cmds.peer_funcs, "get_any_service_client", test_get_any_service_client)
    monkeypatch.setattr(cactus.cmds.cmds_util, "get_wallet_client", test_get_wallet_client)
    monkeypatch.setattr(cactus.cmds.wallet_funcs, "get_wallet_client", test_get_wallet_client)
    monkeypatch.setattr(cactus.cmds.cmds_util, "cli_confirm", cli_confirm)
    monkeypatch.setattr(cactus.cmds.wallet_funcs, "cli_confirm", cli_confirm)


def run_cli_command(capsys: object, cactus_root: Path, command_list: list[str]) -> str:
    """
    This is just an easy way to run the cactus CLI with the given command list.
    """
    # we don't use the real capsys object because its only accessible in a private part of the pytest module
    exited_cleanly = True
    argv_temp = sys.argv
    try:
        sys.argv = ["cactus", "--root-path", str(cactus_root), *command_list]
        cactus_cli()
...
def run_cli_command_and_assert(
    capsys: object, cactus_root: Path, command_list: list[str], strings_to_assert: Iterable[str]
) -> None:
    """
    Runs the command and asserts that all the strings in strings_to_assert are in the output
    """
    output = run_cli_command(capsys, cactus_root, command_list)
