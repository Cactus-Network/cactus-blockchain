...
from cactus._tests.cmds.wallet.test_consts import FINGERPRINT, FINGERPRINT_ARG, STD_TX, STD_UTX, get_bytes32
...
def test_nft_sign_message(capsys: object, get_test_cli_clients: tuple[TestRpcClients, Path]) -> None:
    ...
    expected_calls: logType = {
        "sign_message_by_id": [(nft_id, message.hex())],  # cac std
    }
    test_rpc_clients.wallet_rpc_client.check_log(expected_calls)
    ...
    expected_calls: logType = {
        ...
        "mint_nft": [
            (
                ...
                500000000000,
                0,
                "did:cactus:1qgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpq4msw0c",
                True,
                TXConfig(
...
def test_nft_set_did(capsys: object, get_test_cli_clients: tuple[TestRpcClients, Path]) -> None:
    ...
    did_id = encode_puzzle_hash(get_bytes32(3), "did:cactus:")
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    command_args = [
        "wallet",
        "nft",
...
def test_nft_get_info(capsys: object, get_test_cli_clients: tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Owner DID:                 did:cactus:1qgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpq4msw0c",
    ]
    run_cli_command_and_assert(capsys, root_dir, command_args, assert_list)
    expected_calls: logType = {
