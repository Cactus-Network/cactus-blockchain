...
from cactus._tests.cmds.cmd_test_utils import TestRpcClients, TestWalletRpcClient, logType, run_cli_command_and_assert
from cactus._tests.cmds.wallet.test_consts import (
    CAT_FINGERPRINT_ARG,
    FINGERPRINT,
    FINGERPRINT_ARG,
    STD_TX,
...
from cactus.cmds.cmds_util import TransactionBundle
        ...
        async def get_transactions(self, request: GetTransactions) -> GetTransactionsResponse:
            ...
            for i in range(request.start, request.end):
                ...
                tx_rec = TransactionRecordWithMetadata(
                    ...
                    to_address=encode_puzzle_hash(bytes32([1 + i] * 32), "cac"),
...
def test_show(capsys: object, get_test_cli_clients: tuple[TestRpcClients, Path]) -> None:
    ...
    class ShowRpcClient(TestWalletRpcClient):
        async def get_wallets(self, request: GetWallets) -> GetWalletsResponse:
            ...
            wallet_list: list[WalletInfoResponse] = [
                WalletInfoResponse(
                    data="", id=uint32(1), name="Cactus Wallet", type=uint8(WalletType.STANDARD_WALLET.value)
                ),
                WalletInfoResponse(
        ...
        async def get_nft_wallet_did(self, request: NFTGetWalletDID) -> NFTGetWalletDIDResponse:
            ...
            return NFTGetWalletDIDResponse(
                did_id="did:cactus:1qgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpq4msw0c"
            )
    ...
    assert_list = [
        ...
        "NFT Wallet:\n   -Total Balance:         1.0",
        "   -DID ID:                did:cactus:1qgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpq4msw0c",
        "FULL_NODE 127.0.0.1",
    ...
    class SendWalletRpcClient(TestWalletRpcClient):
        async def send_transaction(
            ...
            tx_rec = TransactionRecord(
                ...
                to_address=encode_puzzle_hash(get_bytes32(1), "cac"),
        ...
        async def get_next_address(self, request: GetNextAddress) -> GetNextAddressResponse:
            ...
            if request.new_address:
                return GetNextAddressResponse(
                    wallet_id=request.wallet_id, address=encode_puzzle_hash(get_bytes32(3), "cac")
                )
            return GetNextAddressResponse(
                wallet_id=request.wallet_id, address=encode_puzzle_hash(get_bytes32(4), "cac")
            )
...
def test_take_offer(capsys: object, get_test_cli_clients: tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        (
            "  REQUESTED:\n"
            "    - CAC (Wallet ID: 1): 2.0 (2000000000000 mojos)\n"
