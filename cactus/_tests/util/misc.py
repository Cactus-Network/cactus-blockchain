...
from cactus._tests.util.time_out_assert import DataTypeProtocol, caller_file_and_line
...
def measure_runtime(
    ...
    entry_file, entry_line = caller_file_and_line(
        relative_to=(
            pathlib.Path(cactus.__file__).parent.parent,
            pathlib.Path(cactus._tests.__file__).parent.parent,
        )
    )
