...
class TestWalletSimulator:
    ...
    async def test_wallet_clawback_claim_auto(
        ...
        tx_amount = 500
        async with env_1.cac_wallet.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
            normal_puzhash = await action_scope.get_puzzle_hash(env_1.cac_wallet.wallet_state_manager)

        # Transfer to normal wallet
        for _ in range(number_of_coins):
            await env.rpc_client.send_transaction(
                SendTransaction(
                    wallet_id=env.cac_wallet.id(),
