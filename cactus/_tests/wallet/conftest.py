...
from cactus._tests.environments.wallet import NewPuzzleHashError, WalletEnvironment, WalletState, WalletTestFramework
...
async def ignore_block_validation(
    ...
    monkeypatch.setattr("cactus.consensus.multiprocess_validation._run_block", run_block)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "cactus.consensus.multiprocess_validation.validate_pospace_and_get_required_iters", lambda *_, **__: uint64(0)
    )
...
async def wallet_environments(
    ...
    async with setup_simulators_and_wallets_service(
        ...
        with unittest.mock.patch(
            "cactus.wallet.wallet_state_manager.WalletStateManager.new_action_scope", new=new_action_scope_wrapped
        ):
