...
async def test_nft_mint(wallet_environments: WalletTestFramework, with_did: bool) -> None:
    ...
    metadata_list = [
        {
            "program": Program.to(
                [("u", ["https://www.cactus-network.net/img/branding/cactus-logo.svg"]), ("h", bytes32.zeros.hex())]
            ),
    ...
    async with nft_wallet_0.wallet_state_manager.new_action_scope(
        ...
        if with_did:
            ...
            await nft_wallet_0.mint_from_cac(
                metadata_list,
                action_scope,
                target_list=target_list,
                mint_number_start=1,
...
async def test_nft_mint_rpc(wallet_environments: WalletTestFramework, zero_royalties: bool, with_did: bool) -> None:
    ...
    target_list = [encode_puzzle_hash(ph_1, "cac") for x in range(n)]
    royalty_address = None if zero_royalties else encode_puzzle_hash(bytes32.zeros, "cac")
    ...
    for i in range(0, n, chunk):
        resp = await env_0.rpc_client.nft_mint_bulk(
            NFTMintBulk(
                ...
                mint_total=uint32(n),
                cac_coins=[next_coin],
                cac_change_target=funding_coin.puzzle_hash.hex(),
...
@pytest.mark.parametrize("with_did", [True, False])
@pytest.mark.anyio
async def test_nft_mint_multiple_cac(wallet_environments: WalletTestFramework, with_did: bool) -> None:
    env_0 = wallet_environments.environments[0]
    env_1 = wallet_environments.environments[1]
    wallet_0 = env_0.cac_wallet
    wallet_1 = env_1.cac_wallet
    ...
    async with env_0.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=False) as action_scope:
        cac_coins_1 = await wallet_0.select_coins(amount=uint64(10000), action_scope=action_scope)
        cac_coins_2 = await wallet_0.select_coins(
            amount=uint64(10000),
