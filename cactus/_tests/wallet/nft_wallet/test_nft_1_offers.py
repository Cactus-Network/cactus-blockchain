...
from cactus.wallet.nft_wallet.nft_wallet import MAX_ROYALTY_BASIS_POINTS, NFTWallet
...
async def test_nft_offer_sell_nft(wallet_environments: WalletTestFramework, zero_royalties: bool) -> None:
    ...
    offer_did_nft_for_cac: OfferSpecification = {nft_to_offer_asset_id: -1, wallet_maker.id(): cac_requested}

    async with trade_manager_maker.wallet_state_manager.new_action_scope(
        wallet_environments.tx_config, push=False
    ) as action_scope:
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            offer_did_nft_for_cac, action_scope, {}, fee=maker_fee
        )
    ...
    await wallet_environments.process_pending_states(
        [
            WalletStateTransition(
                ...
                post_block_balance_updates={
                    "cac": {
                        "confirmed_wallet_balance": (cac_requested + expected_royalty) - maker_fee,
                        "unconfirmed_wallet_balance": (cac_requested + expected_royalty) - maker_fee,
                        ">=#spendable_balance": 1,
                        ">=#max_send_amount": 1,
            ...
            WalletStateTransition(
                pre_block_balance_updates={
                    "cac": {
                        "unconfirmed_wallet_balance": -(taker_fee + cac_requested + expected_royalty),
                        "<=#spendable_balance": -(taker_fee + cac_requested + expected_royalty),
                        "<=#max_send_amount": -(taker_fee + cac_requested + expected_royalty),
                        ">=#pending_change": 1,
                        "pending_coin_removal_count": 1,
                    },
                    "nft": {
                ...
                post_block_balance_updates={
                    "cac": {
                        "confirmed_wallet_balance": -(taker_fee + cac_requested + expected_royalty),
                        ">=#spendable_balance": 1,
                        ">=#max_send_amount": 1,
                        "<=#pending_change": -1,
                        "pending_coin_removal_count": -1,
...
async def test_nft_offer_request_nft(wallet_environments: WalletTestFramework, zero_royalties: bool) -> None:
    ...
    offer_dict: OfferSpecification = {nft_to_request_asset_id: 1, wallet_maker.id(): -cac_offered}
    ...
    await wallet_environments.process_pending_states(
        [
            WalletStateTransition(
                pre_block_balance_updates={
                    "cac": {
                        "<=#spendable_balance": -(maker_fee + cac_offered + expected_royalty),
                        "<=#max_send_amount": -(maker_fee + cac_offered + expected_royalty),
                        "pending_coin_removal_count": 1,
                    },
                    "nft": {
                        "init": True,
                ...
                post_block_balance_updates={
                    "cac": {
                        "confirmed_wallet_balance": -(maker_fee + cac_offered + expected_royalty),
                        "unconfirmed_wallet_balance": -(maker_fee + cac_offered + expected_royalty),
                        ">=#spendable_balance": 1,
                        ">=#max_send_amount": 1,
                        "pending_coin_removal_count": -1,
                    },
            ...
            WalletStateTransition(
                pre_block_balance_updates={
                    "cac": {
                        ...
                        "unconfirmed_wallet_balance": cac_offered - taker_fee,
                ...
                post_block_balance_updates={
                    "cac": {
                        "confirmed_wallet_balance": cac_offered + expected_royalty - taker_fee,
...
async def test_nft_offer_sell_nft_for_cat(
    ...
    offer_did_nft_for_cac: OfferSpecification = {nft_to_offer_asset_id: -1, cat_wallet_maker.id(): cats_requested}
...
async def test_complex_nft_offer(
    ...
    cac_coins = int(XCH_REQUESTED / 1_750_000_000_000) + 2 - (1 if royalty_basis_pts_maker == 0 else 0)
    ...
    await wallet_environments.process_pending_states(
        [
            WalletStateTransition(
                ...
                post_block_balance_updates={
                    "cac": {
                        "unconfirmed_wallet_balance": XCH_REQUESTED + maker_cac_royalties_expected - FEE,
                        "confirmed_wallet_balance": XCH_REQUESTED + maker_cac_royalties_expected - FEE,
                        ">=#spendable_balance": 1,
                        ">=#max_send_amount": 1,
                        "pending_coin_removal_count": -fee_coins,
            ...
            WalletStateTransition(
                pre_block_balance_updates={
                    "cac": {
                        "unconfirmed_wallet_balance": -XCH_REQUESTED - maker_cac_royalties_expected - FEE,
                        "<=#spendable_balance": -XCH_REQUESTED - maker_cac_royalties_expected - FEE,
                        "<=#max_send_amount": -XCH_REQUESTED - maker_cac_royalties_expected - FEE,
                        ">=#pending_change": 0,
                        "pending_coin_removal_count": cac_coins + fee_coins,
                    },
                    "cat_taker": {
                        "unconfirmed_wallet_balance": -CAT_REQUESTED - maker_cat_royalties_expected,
                        "<=#spendable_balance": -CAT_REQUESTED - maker_cat_royalties_expected,
                ...
                post_block_balance_updates={
                    "cac": {
                        ...
                        ">=#max_send_amount": 0,
                        "<=#pending_change": 0,
                        "pending_coin_removal_count": -fee_coins - cac_coins,
                        # Parametrizations make unspent_coin_count too complicated
                        "set_remainder": True,
                    },
                    "cat_taker": {
