...
from cactus.wallet.conditions import AssertPuzzleAnnouncement, ConditionValidTimes, CreateCoin
from cactus.wallet.outer_puzzles import AssetType
from cactus.wallet.puzzle_drivers import PuzzleInfo
from cactus.wallet.trading.offer import OFFER_MOD, Offer
...
async def test_complex_offer(cost_logger: CostLogger) -> None:
    async with sim_and_client() as (sim, sim_client):
        ...
        cactus_requested_payments: dict[bytes32 | None, list[CreateCoin]] = {
            str_to_tail_hash("red"): [
                CreateCoin(acs_ph, uint64(100), [b"memo"]),
                CreateCoin(acs_ph, uint64(200), [b"memo"]),
            ]
        }
        cactus_notarized_payments = Offer.notarize_payments(cactus_requested_payments, cactus_coins)
        cactus_announcements = Offer.calculate_announcements(cactus_notarized_payments, driver_dict)
        cactus_secured_bundle = generate_secure_bundle(cactus_coins, cactus_announcements, uint64(1000))
        ...
        new_offer = Offer.aggregate([cactus_offer, red_offer, red_offer_2])
