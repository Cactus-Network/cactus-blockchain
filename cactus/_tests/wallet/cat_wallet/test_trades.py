...
from cactus._tests.conftest import SOFTFORK_HEIGHTS
...
from cactus._tests.wallet.vc_wallet.test_vc_wallet import mint_cr_cat
...
from cactus.types.blockchain_format.program import INFINITE_COST, Program, run
...
from cactus.wallet.conditions import CreateCoinAnnouncement, parse_conditions_non_consensus
...
from cactus.wallet.trading.offer import Offer, OfferSpecification
...
async def test_cat_trades(
    ...
    if credential_restricted:
                ...
                await client_maker.vc_mint(
                    VCMint(
                        ...
                        target_address=encode_puzzle_hash(
                            await action_scope.get_puzzle_hash(env_maker.wallet_state_manager), "tcac"
                        ),
                        push=True,
                    ),
                    wallet_environments.tx_config,
            ...
            vc_record_taker = (
                await client_taker.vc_mint(
                    VCMint(
                        ...
                        target_address=encode_puzzle_hash(
                            await action_scope.get_puzzle_hash(env_taker.wallet_state_manager), "tcac"
                        ),
                        push=True,
                    ),
                    wallet_environments.tx_config,
        ...
        cat_wallet_maker = await mint_cat(
            wallet_environments, env_maker, "cac", "cat", uint64(100), wallet_type, "cat maker"
        )
        new_cat_wallet_taker = await mint_cat(
            wallet_environments, env_taker, "cac", "new cat", uint64(100), wallet_type, "cat taker"
        )
    ...
    cactus_for_multiple_cat: OfferSpecification = {
        wallet_maker.id(): -7,
        cat_wallet_maker.id(): 8,
        new_cat_wallet_maker.id(): 9,
    }
    multiple_cat_for_cactus: OfferSpecification = {
        wallet_maker.id(): 10,
        cat_wallet_maker.id(): -11,
        new_cat_wallet_maker.id(): -12,
    }
    cactus_and_cat_for_cat: OfferSpecification = {
        wallet_maker.id(): -13,
    ...
    async with trade_manager_maker.wallet_state_manager.new_action_scope(
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            cactus_for_cat, action_scope, fee=uint64(1)
        )
            ...
            WalletStateTransition(
                pre_block_balance_updates={
                    "cac": {
                        ...
                        "<=#spendable_balance": -2,
                        "<=#max_send_amount": -2,
                        # Unconfirmed balance doesn't change because receiving 1 CAC and spending 1 in fee
                        "unconfirmed_wallet_balance": 0,
                ...
                post_block_balance_updates={
                    "cac": {
                        ...
                        ">#spendable_balance": 0,
                        ">#max_send_amount": 0,
                        # Confirmed balance doesn't change because receiving 1 CAC and spending 1 in fee
                        "confirmed_wallet_balance": 0,
    ...
    async with trade_manager_maker.wallet_state_manager.new_action_scope(
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cat_for_cactus, action_scope)
    assert error is None
    assert success is True
    assert trade_make is not None
...
async def test_trade_cancellation(wallet_environments: WalletTestFramework, wallet_type: type[CATWallet]) -> None:
    ...
    async with env_maker.wallet_state_manager.new_action_scope(
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cactus_for_cat, action_scope)
    assert error is None
    assert success is True
    assert trade_make is not None
    ...
    cactus_and_cat_for_something: OfferSpecification = {
    ...
    await wallet_environments.process_pending_states(
        [
            WalletStateTransition(
                pre_block_balance_updates={
                    "cat": {
                        "<=#spendable_balance": cat_for_cactus[env_maker.wallet_aliases["cat"]],
                        "<=#max_send_amount": cat_for_cactus[env_maker.wallet_aliases["cat"]],
                        "pending_change": 0,
                        "pending_coin_removal_count": 1,
                    }
                },
                post_block_balance_updates={
                    ...
                    "cat": {
                        "unconfirmed_wallet_balance": cat_for_cactus[env_maker.wallet_aliases["cat"]],
                        "confirmed_wallet_balance": cat_for_cactus[env_maker.wallet_aliases["cat"]],
                        ">=#spendable_balance": 1,
                        ">=#max_send_amount": 1,
                        "pending_change": 0,
                        "pending_coin_removal_count": -1,
            ...
            WalletStateTransition(
                pre_block_balance_updates={
                    ...
                    "cat": {
                        "init": True,
                        "unconfirmed_wallet_balance": -1 * cat_for_cactus[env_maker.wallet_aliases["cat"]],
                    },
                },
                post_block_balance_updates={
                    "cac": {
                    ...
                    "cat": {
                        "confirmed_wallet_balance": -1 * cat_for_cactus[env_maker.wallet_aliases["cat"]],
                        "spendable_balance": -1 * cat_for_cactus[env_maker.wallet_aliases["cat"]],
                        "max_send_amount": -1 * cat_for_cactus[env_maker.wallet_aliases["cat"]],
                        "unspent_coin_count": 1,
                    },
                },
            ),
...
async def test_trade_bad_spend(
    ...
    monkeypatch.setattr("cactus.wallet.transaction_record.minimum_send_attempts", 1)
    monkeypatch.setattr("cactus.wallet.wallet_transaction_store.minimum_send_attempts", 1)
    env_maker = wallet_environments.environments[0]
    env_taker = wallet_environments.environments[1]
...
async def test_aggregated_trade_state(wallet_environments: WalletTestFramework, wallet_type: type[CATWallet]) -> None:
    ...
    async with trade_manager_maker.wallet_state_manager.new_action_scope(
        ...
        success, trade_make_1, error = await trade_manager_maker.create_offer_for_ids(cactus_for_cat, action_scope)
    ...
    async with trade_manager_maker.wallet_state_manager.new_action_scope(
        ...
        success, trade_make_2, error = await trade_manager_maker.create_offer_for_ids(cat_for_cactus, action_scope)
