...
from cactus_rs import confirm_not_included_already_hashed
from colorlog import getLogger

from cactus._tests.connection_utils import disconnect_all, disconnect_all_and_reconnect
from cactus._tests.util.blockchain_mock import BlockchainMock
from cactus._tests.util.misc import add_blocks_in_batches, wallet_height_at_least
...
async def test_dusted_wallet(
    ...
    for _ in range(large_coins):
        payee_ph = await dust_wallet.get_new_puzzlehash()
        payees.append(Payment(payee_ph, uint64(cac_spam_amount)))
    ...
    for i in range(5):
        ...
        payees.append(Payment(payee_ph, uint64(cac_spam_amount + i)))
        large_coins += 1
        ...
        if cac_spam_amount - dust_value - i > 0:
            payees.append(Payment(payee_ph, uint64(cac_spam_amount - dust_value - i)))
        else:
            payees.append(Payment(payee_ph, uint64(dust_value)))
...
async def test_retry_store(
    ...
    def flaky_get_coin_state(
        ...
        func: Callable[[List[bytes32], WSCactusConnection, Optional[uint32]], Awaitable[List[CoinState]]],
    ) -> Callable[[List[bytes32], WSCactusConnection, Optional[uint32]], Awaitable[List[CoinState]]]:
        async def new_func(
            coin_names: List[bytes32], peer: WSCactusConnection, fork_height: Optional[uint32] = None
        ) -> List[CoinState]:
    ...
    def flaky_fetch_children(
        flakiness_info: FlakinessInfo,
        func: Callable[[bytes32, WSCactusConnection, Optional[uint32]], Awaitable[List[CoinState]]],
    ) -> Callable[[bytes32, WSCactusConnection, Optional[uint32]], Awaitable[List[CoinState]]]:
        async def new_func(
            coin_name: bytes32, peer: WSCactusConnection, fork_height: Optional[uint32] = None
        ) -> List[CoinState]:
