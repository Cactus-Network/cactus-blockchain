...
from cactus._tests.connection_utils import (
...
from cactus._tests.util.blockchain_mock import BlockchainMock
from cactus._tests.util.misc import patch_request_handler, wallet_height_at_least
...
from cactus._tests.weight_proof.test_weight_proof import load_blocks_dont_validate
...
from cactus.full_node.full_node_api import MAX_COIN_HASHES_PER_REQUEST, FullNodeAPI
...
from cactus.wallet.conditions import CreateCoin
...
from cactus.wallet.util.wallet_types import WalletIdentifier
...
async def test_request_block_headers_transactions_filter(
    one_node_one_block: tuple[FullNodeSimulator, CactusServer, BlockTools], rewards_only_tx_block: bool
) -> None:
    """
...
6. Clear all coins from the dust wallet.
    Send to the dust wallet "spam_filter_after_n_txs" coins that are equal in value to "cac_spam_amount".
    Send 1 mojo from the dust wallet. The dust wallet should receive a change coin valued at "cac_spam_amount-1".
...
@pytest.mark.anyio
@pytest.mark.parametrize(
    "spam_filter_after_n_txs, cac_spam_amount, dust_value",
    [
...
async def test_dusted_wallet(
    ...
    # It's also possible to obtain the current settings for spam_filter_after_n_txs and cac_spam_amount
    # spam_filter_after_n_txs = wallets[0][0].config["spam_filter_after_n_txs"]
    # cac_spam_amount = wallets[0][0].config["cac_spam_amount"]
    # dust_value=1
    ...
    # Current max value for cac_spam_amount is 0.01 CAC.
    ...
    assert cac_spam_amount >= 1
    assert cac_spam_amount <= 10_000_000_000
    assert dust_value >= 1
    ...
    # The dust is only filtered at this point if spam_filter_after_n_txs is 0 and cac_spam_amount is > dust_value.
    if spam_filter_after_n_txs > 0:
        dust_coins = 1
        large_dust_coins = 0
        large_dust_balance = 0
    elif cac_spam_amount <= dust_value:
        dust_coins = 0
        large_dust_coins = 1
    ...
    small_unspent_count = len([r for r in all_unspent if r.coin.amount < cac_spam_amount])
    ...
    log.info(f"cac_spam_amount {cac_spam_amount}")
    ...
    async with dust_wallet.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        for _ in range(large_coins):
            ...
            payees.append(CreateCoin(payee_ph, uint64(cac_spam_amount)))
    ...
    large_coin_balance = large_coins * cac_spam_amount

    # Determine whether the filter should have been activated.
    # Make sure the number of coins matches the expected number.
    # At this point, nothing should be getting filtered unless spam_filter_after_n_txs is 0.
    assert dust_coins == spam_filter_after_n_txs
    assert balance == dust_coins * dust_value + large_coins * cac_spam_amount + large_dust_balance
    ...
    async with dust_wallet.wallet_state_manager.new_action_scope(DEFAULT_TX_CONFIG, push=True) as action_scope:
        for i in range(5):
            ...
            payees.append(CreateCoin(payee_ph, uint64(cac_spam_amount + i)))
            large_coins += 1
            large_coin_balance += cac_spam_amount + i

            payee_ph = await action_scope.get_puzzle_hash(dust_wallet.wallet_state_manager)

            # Make sure we are always creating coins with a positive value.
            if cac_spam_amount - dust_value - i > 0:
                payees.append(CreateCoin(payee_ph, uint64(cac_spam_amount - dust_value - i)))
            else:
                payees.append(CreateCoin(payee_ph, uint64(dust_value)))
            # In cases where cac_spam_amount is sufficiently low,
            # the new dust should be considered a large coina and not be filtered.
            if cac_spam_amount <= dust_value:
                large_dust_coins += 1
                large_dust_balance += dust_value
    ...
    # The size of the coin to send the dust wallet is the same as cac_spam_amount
    if cac_spam_amount > 1:
        coin_value = cac_spam_amount
    else:
    ...
    metadata = Program.to(
        [("u", ["https://www.cactus-network.net/img/branding/cactus-logo.svg"]), ("h", "0xD4584AD463139FA8C0D9F68F4B59F185")]
    )
...
async def test_retry_store(
    ...
    def flaky_get_coin_state(
        ...
        func: Callable[[list[bytes32], WSCactusConnection, uint32 | None], Awaitable[list[CoinState]]],
    ) -> Callable[[list[bytes32], WSCactusConnection, uint32 | None], Awaitable[list[CoinState]]]:
        async def new_func(
            coin_names: list[bytes32], peer: WSCactusConnection, fork_height: uint32 | None = None
        ) -> list[CoinState]:
    ...
    def flaky_request_puzzle_solution(
        func: Callable[
            [FullNodeAPI, wallet_protocol.RequestPuzzleSolution, WSCactusConnection], Awaitable[Message | None]
        ],
    ) -> Callable[[FullNodeAPI, wallet_protocol.RequestPuzzleSolution, WSCactusConnection], Awaitable[Message | None]]:
        @functools.wraps(func)
        async def new_func(
            self: FullNodeAPI, request: wallet_protocol.RequestPuzzleSolution, peer: WSCactusConnection
        ) -> Message | None:
    ...
    def flaky_fetch_children(
        flakiness_info: FlakinessInfo,
        func: Callable[[bytes32, WSCactusConnection, uint32 | None], Awaitable[list[CoinState]]],
    ) -> Callable[[bytes32, WSCactusConnection, uint32 | None], Awaitable[list[CoinState]]]:
        async def new_func(
            coin_name: bytes32, peer: WSCactusConnection, fork_height: uint32 | None = None
        ) -> list[CoinState]:
...
async def test_fetch_coin_spend(
    ...
    ) -> Callable[[FullNodeAPI, wallet_protocol.RequestPuzzleSolution, WSCactusConnection], Awaitable[Message]]:
        async def request_puzzle_solution_handler(
            self: FullNodeAPI, request: wallet_protocol.RequestPuzzleSolution, peer_arg: WSCactusConnection
        ) -> Message:
