...
from cactus._tests.environments.wallet import WalletStateTransition, WalletTestFramework
from cactus._tests.util.time_out_assert import time_out_assert
from cactus._tests.wallet.cat_wallet.test_cat_wallet import mint_cat
from cactus._tests.wallet.test_wallet_coin_store import (
...
from cactus.cmds.coins import CombineCMD, SplitCMD
from cactus.cmds.param_types import CliAmount
from cactus.full_node.full_node_rpc_client import FullNodeRpcClient
from cactus.pools.pool_wallet_info import NewPoolWalletInitialTargetState
from cactus.protocols.fee_estimate import FeeEstimate, FeeEstimateGroup
from cactus.protocols.outbound_message import NodeType
from cactus.rpc.rpc_client import ResponseFailureError
...
from cactus.types.fee_rate import FeeRate
from cactus.types.signing_mode import SigningMode
...
from cactus.wallet.cat_wallet.r_cat_wallet import RCATWallet
from cactus.wallet.conditions import (
    ConditionValidTimes,
...
from cactus.wallet.signer_protocol import UnsignedTransaction
from cactus.wallet.trade_record import TradeRecord
from cactus.wallet.trading.offer import Offer, OfferSummary
...
from cactus.wallet.util.blind_signer_tl import BLIND_SIGNER_TRANSLATION
from cactus.wallet.util.clvm_streamable import byte_deserialize_clvm_streamable
from cactus.wallet.util.compute_memos import compute_memos
from cactus.wallet.util.query_filter import AmountFilter, HashFilter, TransactionTypeFilter
from cactus.wallet.util.transaction_type import TransactionType
from cactus.wallet.util.tx_config import TXConfig
from cactus.wallet.util.wallet_sync_utils import PeerRequestException
...
from cactus.wallet.wallet_coin_store import GetCoinRecords
from cactus.wallet.wallet_node import WalletNode, get_wallet_db_path
from cactus.wallet.wallet_protocol import WalletProtocol
from cactus.wallet.wallet_request_types import (
    Addition,
    AddKey,
    CancelOffer,
    CancelOffers,
...
from cactus.wallet.wallet_rpc_client import WalletRpcClient
from cactus.wallet.wallet_spend_bundle import WalletSpendBundle
from cactus.wallet.wallet_state_manager import SyncStatus
...
async def test_send_transaction(wallet_environments: WalletTestFramework) -> None:
    ...
    wallet_2: Wallet = env_2.cac_wallet
    ...
    INITIAL_FUNDS = await env.cac_wallet.get_confirmed_balance()
    async with wallet_2.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=True) as action_scope:
        addr = encode_puzzle_hash(await action_scope.get_puzzle_hash(wallet_2.wallet_state_manager), "tcac")
...
async def test_create_signed_transaction(
    ...
    env.wallet_aliases = {"cac": 1, "cat": 2}
    env_2.wallet_aliases = {"cac": 1, "cat": 2}
    ...
    cac_delta = amount_total if not is_cat else amount_fee
        ...
        [
            WalletStateTransition(
                pre_block_balance_updates={  # type: ignore[arg-type]
                    "cac": {
                        "unconfirmed_wallet_balance": -cac_delta,
                        "<=#spendable_balance": -cac_delta,
                        "<=#max_send_amount": -cac_delta,
                        ">=#pending_change": 0,
                ...
                post_block_balance_updates={  # type: ignore[arg-type]
                    "cac": {
                        "confirmed_wallet_balance": -cac_delta,
                        ">=#spendable_balance": 0,
                        ">=#max_send_amount": 0,
                        "<=#pending_change": 0,
                        "pending_coin_removal_count": -1,
...
async def test_create_signed_transaction_with_excluded_coins(wallet_environments: WalletTestFramework) -> None:
    env = wallet_environments.environments[0]
    wallet_1: Wallet = env.cac_wallet
    wallet_1_rpc: WalletRpcClient = env.rpc_client
    full_node_rpc: FullNodeRpcClient = wallet_environments.full_node_rpc_client
...
async def test_send_transaction_multi(wallet_environments: WalletTestFramework) -> None:
    ...
    INITIAL_BALANCE = await env.cac_wallet.get_confirmed_balance()
...
async def test_get_transactions(wallet_environments: WalletTestFramework) -> None:
    ...
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    await client.send_transaction(
        SendTransaction(wallet_id=uint32(1), amount=uint64(1), address=encode_puzzle_hash(puzhash, "tcac"), push=True),
        wallet_environments.tx_config,
    )  # Create a pending tx
    ...
    await client.send_transaction(
        SendTransaction(
            wallet_id=uint32(1), amount=uint64(1), address=encode_puzzle_hash(ph_by_addr, "tcac"), push=True
        ),
        wallet_environments.tx_config,
    )
    unconfirmed_txs_count += 1
    ...
    tx_for_address = (
        await client.get_transactions(
            GetTransactions(wallet_id=uint32(1), to_address=encode_puzzle_hash(ph_by_addr, "tcac"))
        )
    ).transactions
    assert (
...
async def test_get_coin_records_by_names(wallet_environments: WalletTestFramework) -> None:
    ...
    async with env.wallet_state_manager.new_action_scope(wallet_environments.tx_config, push=True) as action_scope:
        address = encode_puzzle_hash(await action_scope.get_puzzle_hash(env.wallet_state_manager), "tcac")
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
...
async def _check_delete_key(
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["cac_target_address"] = encode_puzzle_hash(farmer_ph, "tcac")
        test_config["pool"]["cac_target_address"] = encode_puzzle_hash(pool_ph, "tcac")
...
async def test_key_and_address_endpoints(wallet_environments: WalletTestFramework) -> None:
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["cac_target_address"] = ""
        save_config(wallet_node.root_path, "config.yaml", test_config)
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["cac_target_address"] = ""
        test_config["pool"]["cac_target_address"] = ""
        save_config(wallet_node.root_path, "config.yaml", test_config)
...
async def test_select_coins_rpc(wallet_environments: WalletTestFramework) -> None:
    ...
    env.wallet_aliases = {"cac": 1}
    env_2.wallet_aliases = {"cac": 1}

    wallet_2: Wallet = env_2.cac_wallet
    client: WalletRpcClient = env.rpc_client
    client_2: WalletRpcClient = env_2.rpc_client

    funds: int = await env.cac_wallet.get_confirmed_balance()
...
# $ cactus keys sign -d 'hello world' -t 'm/12381/8444/1/1'
#
# Example 2:
# $ cactus wallet get_address
# cac1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
# $ cactus wallet sign_message -m $(echo -n 'hello world' | xxd -p)
# -a cac1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
#
@pytest.mark.parametrize(
    ...
    [
        # Valid signatures
        (
            # cactus keys sign -d "Let's eat, Grandma" -t "m/12381/8444/1/1"
            {
        ...
        (
            # cactus wallet sign_message -m $(echo -n 'Happy happy joy joy' | xxd -p)
            # -a cac1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
        ...
        (
            ...
            # -a cac1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "cac1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh",
            },
            VerifySignatureResponse(isValid=True),
        ),
        (
            {
                ...
                "address": "cac1hh9phcc8tt703dla70qthlhrxswy88va04zvc7vd8cx2v6a5ywyst8mgul",
            },
            VerifySignatureResponse(isValid=True),
        ),
        # Negative tests
        ...
        (
            # Valid signature but address doesn't match pubkey
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "cac1d0rekc2javy5gpruzmcnk4e4qq834jzlvxt5tcgl2ylt49t26gdsjen7t0",
            },
...
async def test_set_wallet_resync_on_startup(wallet_environments: WalletTestFramework) -> None:
    ...
    address = (await wc.get_next_address(GetNextAddress(wallet_id=env.cac_wallet.id(), new_address=True))).address
    await wc.mint_nft(
        request=NFTMintNFTRequest(
            wallet_id=nft_wallet_id,
    ...
    tx = (
        await client.cat_spend(
            CATSpend(
                ...
                amount=uint64(0),
                inner_address=encode_puzzle_hash(our_ph, "tcac"),
...
async def test_split_coins(wallet_environments: WalletTestFramework, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    async with env.wallet_state_manager.new_action_scope(wallet_environments.tx_config) as action_scope:
        target_coin = next(iter(await env.cac_wallet.select_coins(uint64(250_000_000_000), action_scope)))
        assert target_coin.amount == 250_000_000_000

    cac_request = SplitCMD(
        **{
            **wallet_environments.cmd_tx_endpoint_args(env),
            **dict(
                id=env.wallet_aliases["cac"],
                number_of_coins=100,
                amount_per_coin=CliAmount(amount=uint64(100), mojos=True),
                target_coin_id=target_coin.name(),
                fee=uint64(1_000_000_000_000),  # 1 CAC
                push=True,
            ),
        }
    )
    ...
    with pytest.raises(ValueError, match="501 coins is greater then the maximum limit of 500 coins"):
        await dataclasses.replace(cac_request, number_of_coins=501).run()

    with pytest.raises(ValueError, match="Cannot split into 0 new coins"):
        await dataclasses.replace(cac_request, number_of_coins=0).run()

    with pytest.raises(ResponseFailureError, match="Could not find coin with ID 00000000000000000"):
        await dataclasses.replace(cac_request, target_coin_id=bytes32.zeros).run()

    with pytest.raises(ResponseFailureError, match="is less than the total amount of the split"):
        await dataclasses.replace(
            cac_request, amount_per_coin=CliAmount(amount=uint64(1_000_000_000_000), mojos=True)
        ).run()

    # We catch this one
    capsys.readouterr()
    await dataclasses.replace(cac_request, id=50).run()
    ...
    with pytest.raises(ResponseFailureError, match="Cannot split coins from non-fungible wallet types"):
        assert cac_request.amount_per_coin is not None  # hey there mypy
        rpc_request = SplitCoins(
            wallet_id=uint32(42),
            number_of_coins=uint16(cac_request.number_of_coins),
            amount_per_coin=cac_request.amount_per_coin.convert_amount(1),
            target_coin_id=cac_request.target_coin_id,
            fee=cac_request.fee,
            push=cac_request.push,
        )
    ...
    assert cac_request.rpc_info.client_info is not None

    async def not_synced() -> GetSyncStatusResponse:
        return GetSyncStatusResponse(synced=False, syncing=False)

    cac_request.rpc_info.client_info.client.get_sync_status = not_synced  # type: ignore[method-assign]
    await cac_request.run()
    output = (capsys.readouterr()).out
...
async def test_combine_coins(wallet_environments: WalletTestFramework, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    # Should have 4 coins, two 1.75 CAC, two 0.25 CAC
    ...
    fee = uint64(100)
    cac_combine_request = CombineCMD(
        **{
            **wallet_environments.cmd_tx_endpoint_args(env),
            **dict(
                id=env.wallet_aliases["cac"],
    ...
    with pytest.raises(ValueError, match="greater then the maximum limit"):
        await dataclasses.replace(cac_combine_request, number_of_coins=uint16(501)).run()

    with pytest.raises(ValueError, match="You need at least two coins to combine"):
        await dataclasses.replace(cac_combine_request, number_of_coins=uint16(0)).run()

    with pytest.raises(ValueError, match="More coin IDs specified than desired number of coins to combine"):
        await dataclasses.replace(cac_combine_request, input_coins=(bytes32.zeros,) * 100).run()

    # We catch this one
    capsys.readouterr()
    await dataclasses.replace(cac_combine_request, id=50).run()
    output = (capsys.readouterr()).out
    assert "Wallet id: 50 not found" in output
    ...
    with pytest.raises(ResponseFailureError, match="Cannot combine coins from non-fungible wallet types"):
        assert cac_combine_request.target_amount is not None  # hey there mypy
        rpc_request = CombineCoins(
            wallet_id=uint32(42),
            target_coin_amount=cac_combine_request.target_amount.convert_amount(1),
            number_of_coins=uint16(cac_combine_request.number_of_coins),
            target_coin_ids=list(cac_combine_request.input_coins),
            fee=cac_combine_request.fee,
            push=cac_combine_request.push,
        )
    ...
    # Test a not synced error
    assert cac_combine_request.rpc_info.client_info is not None

    async def not_synced() -> GetSyncStatusResponse:
        return GetSyncStatusResponse(synced=False, syncing=False)

    cac_combine_request.rpc_info.client_info.client.get_sync_status = not_synced  # type: ignore[method-assign]
    await cac_combine_request.run()
    output = (capsys.readouterr()).out
    assert "Wallet not synced. Please wait." in output
...
async def test_fee_bigger_than_selection_coin_combining(wallet_environments: WalletTestFramework) -> None:
    ...
    with pytest.raises(ResponseFailureError, match="without selecting more coins than specified: 3"):
        await dataclasses.replace(cac_combine_request, fee=uint64(2_250_000_000_000)).run()
...
async def test_combine_coins_respects_max_coin_amount(wallet_environments: WalletTestFramework) -> None:
    ...
    Setup gives 4 coins: 2x 1.75 CAC + 2x 0.25 CAC.
    With largest_first=True and NO filter, the two 1.75 CAC coins would be
    selected.  With max_coin_amount=0.25 CAC the 1.75 CAC coins must be
    excluded and only the 0.25 CAC coins selected -- proving the filter works.
    """
    env = wallet_environments.environments[0]
    env.wallet_aliases = {"cac": 1}

    SMALL = uint64(250_000_000_000)  # 0.25 CAC
...
async def test_combine_coins_rejects_spent_coins(wallet_environments: WalletTestFramework) -> None:
    ...
    # Grab a 0.25 CAC coin and remember its ID before spending it
    async with env.wallet_state_manager.new_action_scope(wallet_environments.tx_config) as action_scope:
        target_coin = next(iter(await env.cac_wallet.select_coins(SMALL, action_scope)))
    ...
    # Wallet now has 3 unspent coins (2x 1.75 CAC + 1x 0.5 CAC) and 2 spent
    # 0.25 CAC records still in the DB.  A fill-up combine (no explicit IDs,
    # smallest-first) must skip the spent records and select only unspent coins.
    # Without the spent_range fix this would silently pick the spent 0.25 CAC
    # records because they sort before the 0.5 CAC coin.
    COMBINED = uint64(2 * SMALL)  # 0.5 CAC from first combine
    BIG = uint64(1_750_000_000_000)  # 1.75 CAC
...
def test_send_transaction_multi_post_init() -> None:
    with pytest.raises(
        ValueError,
        match=re.escape('Specified "morph_bytes" for a CAT-type wallet. Maybe you meant to specify an CAC wallet?'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
            morph_bytes=b"",
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            'Specified "coin/puzzle_announcements" for a CAT-type wallet.Maybe you meant to specify an CAC wallet?'
        ),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('Specified "amount" for an CAC wallet. Maybe you meant to specify a CAT-type wallet?'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
            amount=uint64(100),
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('Specified "inner_address" for an CAC wallet. Maybe you meant to specify a CAT-type wallet?'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
            inner_address="foo",
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('Specified "memos" for an CAC wallet. Maybe you meant to specify a CAT-type wallet?'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
            memos=["foo"],
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            'Specified "extra_delta", "tail_reveal", or "tail_solution" for an CAC wallet.'
    ...
    with pytest.raises(
        ValueError,
        match=re.escape('"additions" are required for CAC wallets.'),
    ):
        SendTransactionMulti(
            wallet_id=uint32(1),
        ).convert_to_proxy(CreateSignedTransaction)
