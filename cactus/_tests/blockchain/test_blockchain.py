...
from cactus._tests.blockchain.blockchain_test_utils import (
...
from cactus.consensus.block_body_validation import ForkAdd, ForkInfo
...
from cactus.consensus.coinbase import create_farmer_coin
from cactus.consensus.find_fork_point import lookup_fork_chain
...
from cactus.types.blockchain_format.vdf import VDFInfo, VDFProof, validate_vdf
...
class TestGenesisBlock:
    ...
    async def test_genesis_validate_1(
        ...
        monkeypatch.setattr(
            "cactus.consensus.multiprocess_validation.pre_sp_tx_block_height",
...
class TestBlockHeaderValidation:
    ...
    # --- a/cactus/consensus/multiprocess_validation.py
    # +++ b/cactus/consensus/multiprocess_validation.py
    ...
    async def test_height(self, empty_blockchain: Blockchain, bt: BlockTools, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "cactus.consensus.blockchain_mmr.BlockchainMMRManager.add_block_to_mmr",
...
class TestReorgs:
    ...
    async def test_long_reorg(
        ...
        monkeypatch.setattr("cactus.consensus.block_header_validation.validate_vdf", lambda *a, **kw: True)
        monkeypatch.setattr("cactus.consensus.block_header_validation.AugSchemeMPL.verify", lambda *a, **kw: True)
        if light_blocks:
...
class ForkInfoTestSetup:
    ...
    def create(cls, same_ph_as_parent: bool, same_amount_as_parent: bool) -> ForkInfoTestSetup:
        from cactus._tests.util.network_protocol_data import full_block as test_block
