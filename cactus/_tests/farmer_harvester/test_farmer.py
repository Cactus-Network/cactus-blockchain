...
from cactus._tests.util.misc import DataCase, Marks, datacases
from cactus.consensus.default_constants import DEFAULT_CONSTANTS
from cactus.farmer.farmer import UPDATE_POOL_FARMER_INFO_INTERVAL, Farmer, increment_pool_stats, strip_old_entries
...
async def test_farmer_pool_response(
    ...
    peer = cast(WSCactusConnection, DummyHarvesterPeer(False))
    await farmer_api.new_proof_of_space(new_pos, peer)
...
@datacases(
    PartialSubmitHeaderCase(
        ...
        expected_headers={
            ...
            "cactus-harvester-version": "1.2.3.asdf42",
        },
    ),
)
@pytest.mark.anyio
async def test_farmer_additional_headers_on_partial_submit(
    ...
    peer = cast(WSCactusConnection, case.harvester_peer)
    await farmer_api.new_proof_of_space(new_pos, peer)
