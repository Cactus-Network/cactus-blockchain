...
from cactus._tests.util.time_out_assert import time_out_assert_not_none
from cactus.daemon.client import DaemonProxy, connect_to_daemon
...
@dataclass
class CactusPlottersBladebitArgsCase:
    case_id: str
    plot_type: str
    count: int = 1
    threads: int = 0
    ...
    async with aiohttp.ClientSession() as client:
        async with client.ws_connect(
            ...
            async with setup_full_node(
                ...
                await time_out_assert_not_none(30, ws_server.connections.get, "cactus_full_node")

                payload = create_payload("get_blockchain_state", {}, service_name, "cactus_full_node")
                await ws.send_str(payload)

                response = await ws.receive()
                message = assert_response_success_only(response)
                assert message["command"] == "get_blockchain_state"
                assert message["origin"] == "cactus_full_node"
...
@datacases(
    CactusPlottersBladebitArgsCase(case_id="1", plot_type="cudaplot"),
    CactusPlottersBladebitArgsCase(case_id="2", plot_type="cudaplot", hybrid_disk_mode=16),
    CactusPlottersBladebitArgsCase(case_id="3", plot_type="cudaplot", hybrid_disk_mode=128),
)
@pytest.mark.limit_consensus_modes(allowed=[ConsensusMode.PLAIN], reason="not relevant")
def test_run_plotter_bladebit(
    ...
    mock_daemon_with_config_and_keys,
    bt: BlockTools,
    case: CactusPlottersBladebitArgsCase,
) -> None:
    ...
    mocker.patch("cactus.plotters.bladebit.get_bladebit_version", side_effect=get_bladebit_version)
    mock_run_plotter = mocker.patch("cactus.plotters.bladebit.run_plotter")
...
async def test_message_logging_redaction(
    ...
    with caplog.at_level(logging.DEBUG, logger="cactus.daemon.server"):
