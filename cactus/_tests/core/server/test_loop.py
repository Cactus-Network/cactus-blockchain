...
from cactus._tests.core.server import serve
from cactus._tests.util.misc import create_logger
...
async def test_loop(tmp_path: pathlib.Path) -> None:
    ...
    with subprocess.Popen(
        [sys.executable, "-m", "cactus._tests.core.server.serve", os.fspath(serve_file)],
    ):
        ...
        with subprocess.Popen(
            [sys.executable, "-m", "cactus._tests.core.server.flood", os.fspath(flood_file)],
        ):
