...
from cactus._tests.connection_utils import add_dummy_connection_wsc, connect_and_get_peer
...
from cactus.full_node.start_full_node import create_full_node_service
from cactus.protocols.full_node_protocol import RejectBlock, RequestBlock, RequestTransaction
...
from cactus.protocols.shared_protocol import Capability, Error, protocol_version
from cactus.protocols.wallet_protocol import RejectHeaderRequest
...
from cactus.server.ws_connection import WSCactusConnection, error_response_version, sanitize_version_string
...
from cactus.util.errors import ApiError, Err
from cactus.util.task_referencer import create_referenced_task
from cactus.wallet.start_wallet import create_wallet_service
...
async def test_duplicate_client_connection(
    two_nodes: tuple[FullNodeAPI, FullNodeAPI, CactusServer, CactusServer, BlockTools], self_hostname: str
) -> None:
...
async def test_start_client_handshake_timeout(
    ...
    async def timeout_handshake(
        self: WSCactusConnection, network_id: str, server_port: uint16, local_type: NodeType
    ) -> None:
        raise asyncio.TimeoutError

    monkeypatch.setattr(WSCactusConnection, "perform_handshake", timeout_handshake)
...
async def test_start_client_handshake_timeout_configurable(
    ...
    monkeypatch.setattr("cactus.server.server.is_localhost", lambda _host: False)
...
async def test_connection_string_conversion(
    two_nodes_one_block: tuple[FullNodeAPI, FullNodeAPI, CactusServer, CactusServer, BlockTools],
    self_hostname: str,
...
async def test_error_receive(
    ...
    def error_log_found(connection: WSCactusConnection) -> bool:
...
async def test_garbage_collect_connections_tolerates_stale_closed_connections(
    ...
    cactus_ca_crt, cactus_ca_key = cactus_ssl_ca_paths(bt.root_path, bt.config)
    my_test_server = CactusServer.create(
        port=None,
        node=None,
        ...
        cactus_ca_crt_key=(cactus_ca_crt, cactus_ca_key),
        stub_metadata_for_type={},
        name="Hello",
    )
    missing_connection = FakeConnection(PeerInfo("10.1.1.1", 11444), bytes32(b"\x01" * 32))
    stale_connection = FakeConnection(PeerInfo("10.1.1.2", 11444), bytes32(b"\x02" * 32))
    replacement_connection = FakeConnection(stale_connection.peer_info, stale_connection.peer_node_id, closed=False)
    remaining_connection = FakeConnection(PeerInfo("10.1.1.3", 11444), bytes32(b"\x03" * 32))
    missing_wsc = cast(WSCactusConnection, missing_connection)
    stale_wsc = cast(WSCactusConnection, stale_connection)
    replacement_wsc = cast(WSCactusConnection, replacement_connection)
    remaining_wsc = cast(WSCactusConnection, remaining_connection)
...
async def test_connection_closed_banning(bt: BlockTools, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING):
        ...
        exempt_peer = FakeConnection(peer_info=PeerInfo("10.1.1.1", 11444))
        await my_test_server.connection_closed(cast(WSCactusConnection, exempt_peer), ban_time=60)
        ...
        localhost_peer = FakeConnection(peer_info=PeerInfo("127.0.0.1", 11444))
        await my_test_server.connection_closed(cast(WSCactusConnection, localhost_peer), ban_time=60)
        assert "Trying to ban localhost for 60, but will not ban" in caplog.text

        # testing trusted peers exemption
        trusted_peer = FakeConnection(peer_info=PeerInfo("34.34.34.34", 11444))
        await my_test_server.connection_closed(cast(WSCactusConnection, trusted_peer), ban_time=60)
...
async def test_inbound_handshake_timeout(
    ...
    cactus_ca_crt_path, cactus_ca_key_path = cactus_ssl_ca_paths(server_1.root_path, config)

    dummy_crt_path = server_1.root_path / "dummy_slowloris.crt"
    dummy_key_path = server_1.root_path / "dummy_slowloris.key"
    generate_ca_signed_cert(
        cactus_ca_crt_path.read_bytes(),
        cactus_ca_key_path.read_bytes(),
        dummy_crt_path,
        dummy_key_path,
    )
    ssl_context = ssl_context_for_client(cactus_ca_crt_path, cactus_ca_key_path, dummy_crt_path, dummy_key_path)

    session = aiohttp.ClientSession()
    try:
        url = f"wss://{self_hostname}:{server_1._port}/ws"
        with patch("cactus.server.server.is_localhost", return_value=False):
