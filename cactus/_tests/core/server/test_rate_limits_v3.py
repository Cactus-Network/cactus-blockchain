...
from cactus._tests.util import network_protocol_data
...
from cactus.protocols.shared_protocol import Capability, ConfigureWindowSizes, default_capabilities
from cactus.server.rate_limits_v3 import (
...
async def test_invalid_config_settings_length(
    ...
    monkeypatch.setattr(
        "cactus.server.ws_connection.rl_v3_to_configure_message", lambda: ConfigureWindowSizes(settings=settings)
    )
...
async def test_malformed_config_message_bytes(
    ...
    original_send_message = WSCactusConnection._send_message

    async def test_send_message(self: WSCactusConnection, message: Message, priority: int = 0) -> None:
    ...
    monkeypatch.setattr(WSCactusConnection, "_send_message", test_send_message)
