...
from cactus.protocols.shared_protocol import Capability, Handshake, default_capabilities, protocol_version
from cactus.server.rate_limits import RateLimiter
from cactus.server.server import CactusServer, ssl_context_for_client
from cactus.server.ssl_context import cactus_ssl_ca_paths
from cactus.server.ws_connection import (
...
async def send_handshake_and_assert_protocol_error(
    server_1: CactusServer,
    server_2: CactusServer,
    self_hostname: str,
...
class TestDos:
    ...
    async def test_large_message_disconnect_and_ban(
        ...
        async with (
            ...
            with monkeypatch.context() as monkey_patch_context:
                monkey_patch_context.setattr(cactus.server.server, "is_localhost", not_localhost)
    ...
    async def test_spam_tx(
        ...
        ws_con: WSCactusConnection = next(iter(server_1.all_connections.values()))
        ws_con_2: WSCactusConnection = next(iter(server_2.all_connections.values()))
    ...
    async def test_invalid_message_format_disconnect(
        self, one_node_one_block: tuple[FullNodeSimulator, CactusServer, BlockTools], self_hostname: str
    ) -> None:
        ...
        ca_crt_path, ca_key_path = cactus_ssl_ca_paths(server.root_path, load_config(server.root_path, "config.yaml"))
