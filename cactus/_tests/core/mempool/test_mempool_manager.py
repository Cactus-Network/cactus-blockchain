...
from cactus._tests.util.misc import Marks, datacases, invariant_check_mempool
from cactus._tests.util.setup_nodes import OldSimulatorsAndWallets, setup_simulators_and_wallets
...
from cactus.full_node.eligible_coin_spends import (
    DedupCoinSpend,
    IdenticalSpendDedup,
    SkipDedup,
)
from cactus.full_node.mempool import MAX_SKIPPED_ITEMS, PRIORITY_TX_THRESHOLD
...
from cactus.protocols.full_node_protocol import RequestBlock, RespondBlock
...
from cactus.types.blockchain_format.program import DEFAULT_FLAGS, INFINITE_COST, Program
from cactus.types.blockchain_format.serialized_program import SerializedProgram
from cactus.types.clvm_cost import QUOTE_BYTES, QUOTE_EXECUTION_COST, CLVMCost
...
from cactus.types.mempool_item import BundleCoinSpend, MempoolItem, UnspentLineageInfo
...
from cactus.wallet.conditions import AssertCoinAnnouncement
...
async def test_identical_spend_aggregation_e2e(
    ...
    async def send_to_mempool(
        ...
        wallet_peer: WSCactusConnection,
...
async def test_fill_rate_block_validation(
    ...
    async def send_to_mempool(
        full_node: FullNodeSimulator, dummy_peer: WSCactusConnection, spend_bundle: SpendBundle
    ) -> None:
...
def make_singleton_spend(
    ...
    from cactus.wallet.puzzles.singleton_top_layer_v1_1 import puzzle_for_singleton, solution_for_singleton
