...
from cactus.cmds.wallet_funcs import print_offer_summary
from cactus.wallet.wallet_request_types import CATAssetIDToName, CATAssetIDToNameResponse
...
@pytest.mark.anyio
async def test_print_offer_summary_cac(capsys: Any) -> None:
    summary_dict = {"cac": str(1_000_000_000_000)}

    await print_offer_summary(cat_name_resolver, summary_dict)

    captured = capsys.readouterr()

    assert "CAC (Wallet ID: 1): 1.0 (1000000000000 mojos)" in captured.out
...
@pytest.mark.anyio
async def test_print_offer_summary_cac_and_cats(capsys: Any) -> None:
    summary_dict = {
        "cac": str(2_500_000_000_000),
    ...
    assert "CAC (Wallet ID: 1): 2.5 (2500000000000 mojos)" in captured.out
...
@pytest.mark.anyio
async def test_print_offer_summary_cac_and_cats_with_zero_values(capsys: Any) -> None:
    summary_dict = {
        "cac": str(0),
    ...
    assert "CAC (Wallet ID: 1): 0.0 (0 mojos)" in captured.out
...
@pytest.mark.anyio
async def test_print_offer_summary_cac_with_one_mojo(capsys: Any) -> None:
    summary_dict = {"cac": str(1)}

    await print_offer_summary(cat_name_resolver, summary_dict)

    captured = capsys.readouterr()

    assert "CAC (Wallet ID: 1): 1e-12 (1 mojo)" in captured.out
