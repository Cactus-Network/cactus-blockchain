...
@pytest.mark.anyio
async def test_help(cactus_root: CactusRoot) -> None:
    """Just a trivial test to make sure the subprocessing is at least working and the
    data executable does run.
    """
    completed_process = cactus_root.run(args=["data", "--help"])
    assert "Show this message and exit" in completed_process.stdout


@pytest.mark.skip
@pytest.mark.anyio
def test_round_trip(cactus_root: CactusRoot, cactus_daemon: None, cactus_data: None) -> None:
    """Create a table, insert a row, get the row by its hash."""

    with cactus_root.print_log_after():
        create = cactus_root.run(args=["data", "create_data_store"])
