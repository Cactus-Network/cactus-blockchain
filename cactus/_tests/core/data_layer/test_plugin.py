...
from cactus.data_layer.data_layer import get_plugin_info
from cactus.data_layer.data_layer_util import PluginRemote, ServerInfo
from cactus.data_layer.download_data import download_file
from cactus.data_layer.util.plugin import load_plugin_configurations
...
async def _mock_session_post(response: MagicMock) -> AsyncIterator[MagicMock]:
    ...
    with patch("cactus.data_layer.data_layer.aiohttp.ClientSession", return_value=session):
        yield session
...
async def _mock_session_post_download(response: MagicMock) -> AsyncIterator[MagicMock]:
    ...
    with patch("cactus.data_layer.download_data.aiohttp.ClientSession", return_value=session):
        yield session
...
async def test_get_plugin_info_timeout() -> None:
    ...
    with patch(
        "cactus.data_layer.data_layer.aiohttp.ClientSession",
...
async def test_download_file_plugin_timeout(tmp_path: Path) -> None:
    ...
    with patch(
        "cactus.data_layer.download_data.aiohttp.ClientSession",
