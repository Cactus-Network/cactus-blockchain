...
from cactus.data_layer.data_layer_util import InternalNode, Node, NodeType, Side, Status, TerminalNode
...
class CactusRoot:
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        kwargs["env"]["CACTUS_ROOT"] = os.fspath(self.path)
        kwargs["env"]["CACTUS_KEYS_ROOT"] = os.fspath(self.path)
        ...
        modified_args: List[Union[str, os_PathLike_str]] = [
            self.scripts_path.joinpath(cactus_executable),
            "--root-path",
            self.path,
            *args,
        ]
