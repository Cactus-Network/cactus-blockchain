...
class PausableServer(BaseEventsServer):
    ...
    def _attach(self, *args: object, **kwargs: object) -> None:
        ...
        active_connections = self._cactus_active_connections()
    ...
    def _cactus_active_connections(self) -> int:
        if sys.version_info >= (3, 13):
...
async def _cactus_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class CactusSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _cactus_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class CactusProactor(IocpProactor):
        ...
        async def _cactus_accept_loop(self, listener: socket.socket) -> tuple[socket.socket, tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._cactus_accept(listener)
        ...
        def _cactus_accept(self, listener: socket.socket) -> asyncio.Future[tuple[socket.socket, tuple[object, ...]]]:
            ...
            async def accept_coro(self: CactusProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[tuple[socket.socket, tuple[object, ...]]]:
            coro = self._cactus_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class CactusProactorEventLoop(ProactorEventLoop):
...
if sys.version_info >= (3, 14):
    ...
    class CactusPolicy(asyncio.events._BaseDefaultEventLoopPolicy):
        def new_event_loop(self) -> asyncio.AbstractEventLoop:
            # overriding https://github.com/python/cpython/blob/v3.14.0/Lib/asyncio/events.py#L726-L732
            if sys.platform == "win32":
                loop_factory = CactusProactorEventLoop
            else:
                loop_factory = CactusSelectorEventLoop

            return loop_factory()
else:

    class CactusPolicy(asyncio.DefaultEventLoopPolicy):
        def new_event_loop(self) -> asyncio.AbstractEventLoop:
...
def set_cactus_policy(connection_limit: int) -> None:
