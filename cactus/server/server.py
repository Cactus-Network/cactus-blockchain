...
from cactus.util.api_decorators import get_metadata
from cactus.util.errors import Err, ProtocolError
from cactus.util.ints import uint16
from cactus.util.network import WebServer, is_in_network, is_localhost
from cactus.util.ssl_check import verify_ssl_certs_and_keys

max_message_size = 50 * 1024 * 1024  # 50MB

ConnectionCallback = Callable[[WSCactusConnection], Awaitable[None]]
...
class CactusServer:
    ...
    incoming_messages: asyncio.Queue[Tuple[Message, WSCactusConnection]] = field(default_factory=asyncio.Queue)
    ...
    def create(
        ...
        cactus_ca_crt_path, cactus_ca_key_path = cactus_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                cactus_ca_crt_path, cactus_ca_key_path, public_cert_path, public_key_path
            )
        ...
        if local_type in authenticated_server_types:
            ...
            public_cert_path, public_key_path = public_ssl_paths(root_path, config)
            ssl_context = ssl_context_for_server(
                cactus_ca_crt_path, cactus_ca_key_path, public_cert_path, public_key_path, log=log
            )
    ...
    def connection_closed(self, connection: WSCactusConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    async def incoming_api_task(self) -> None:
        while True:
            ...
            async def api_call(full_message: Message, connection: WSCactusConnection, task_id: bytes32) -> None:
