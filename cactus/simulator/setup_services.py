...
from cactus.util.config import config_path_for_filename, lock_and_load_config, save_config
...
async def setup_wallet_node(
    ...
    with TempKeyring(populate=True) as keychain:
        ...
        service_config["_spam_amount"] = _spam_amount
...
async def setup_farmer(
    ...
    service_config["_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "cac")
    service_config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    service_config["port"] = port
    service_config["rpc_port"] = uint16(0)
    config_pool["_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "cac")
