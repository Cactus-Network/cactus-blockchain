...
from cactus.cmds.init_funcs import check_keys, cactus_init
...
from cactus.util.keychain import Keychain, KeyData, passphrase_requirements, supports_os_passphrase_storage
from cactus.util.lock import Lockfile, LockfileError
from cactus.util.log_exceptions import log_exceptions
from cactus.util.network import WebServer
...
if getattr(sys, "frozen", False):
    name_map = {
        "cactus": "cactus",
        "cactus_data_layer": "start_data_layer",
        "cactus_data_layer_http": "start_data_layer_http",
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        if plotter == "bladebit":
            ...
            # 'cactus plotters bladebit ramplot' or 'cactus plotters bladebit diskplot'
            plot_type = request["plot_type"]
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with CACTUS_ROOT: {os.environ['CACTUS_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"cactus-blockchain version: {cactus_short_version()}")

            beta_metrics = None
            if config.get("beta", {}).get("enabled", False):
                from cactus.util.beta_metrics import BetaMetricsLogger
