...
from cactus.cmds.cmd_classes import CactusCliContext, cactus_command, option
from cactus.cmds.cmd_helpers import NeedsWalletRPC
from cactus.data_layer.data_layer import server_files_path_from_config
from cactus.data_layer.data_layer_util import ServerInfo, Status, Subscription
from cactus.data_layer.data_store import DataStore, default_prefer_file_kv_blob_length
from cactus.data_layer.download_data import insert_from_delta_file
...
from cactus.wallet.wallet_request_types import DLHistory, DLTrackNew
...
class SyncTimeCommand:
    ...
    context: CactusCliContext = field(default_factory=CactusCliContext)
    ...
    async def run(self) -> None:
        ...
        if self.restart_all:
            await self.run_cactus("stop", "-d", "all", check=False)

        await self.run_cactus("start", "wallet")
        await self.run_cactus("keys", "generate", "--label", "for_testing")
        await self.wait_for_wallet_synced()
        await self.run_cactus("wallet", "show")
    ...
    async def wait_for_wallet_synced(self) -> None:
        ...
        while True:
            run_result = await self.run_cactus(
                "wallet",
                "show",
    ...
    async def run_cactus(self, *args: str, check: bool = True, **kwargs: Any) -> RunResult:
        env = os.environ.copy()
        venv_path = Path(sysconfig.get_path("scripts"))
        env["PATH"] = os.pathsep.join([os.fspath(venv_path), env["PATH"]])
        env["CACTUS_DATA_LAYER_STOP_AFTER_GENERATION"] = str(self.generation_limit)
        env["CACTUS_ROOT"] = os.fspath(self.context.root_path)
        env["CACTUS_KEYS_ROOT"] = os.fspath(self.context.keys_root_path)
