...
from cactus.cmds.cmd_classes import cactus_command, option
...
class TestCMD:
    ...
    owner: str = option("-w", "--owner", help="Owner of the repo", type=str, default="Cactus-Network")
    repository: str = option("-r", "--repository", help="Repository name", type=str, default="cactus-blockchain")
    ref: str | None = option(
        "-f",
        "--ref",
    ...
    async def run(self) -> None:
        ...
        if self.ref is not None:
            ...
            run_url = f"https://github.com/Cactus-Network/cactus-blockchain/actions/workflows/test.yml?query={urllib.parse.quote(query)}"
    ...
    async def check_only(self) -> None:
        if self.only is not None:
            ...
            test_path = Path(cactus._tests.__file__).parent
