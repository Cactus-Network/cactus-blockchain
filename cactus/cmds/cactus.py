...
from cactus.cmds.dev.main import dev_cmd
...
from cactus.cmds.solver import solver_cmd
...
from cactus.util.default_root import DEFAULT_KEYS_ROOT_PATH, resolve_root_path
from cactus.util.errors import KeychainCurrentPassphraseIsInvalid
from cactus.util.keychain import Keychain, set_keys_root_path
...
def cli(
    ...
    context = CactusCliContext.set_default(ctx=ctx)
...
@cli.command("version", help="Show cactus version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", help="Runs cactus daemon")
@click.option(
    "--wait-for-unlock",
...
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    ...
    asyncio.run(async_run_daemon(CactusCliContext.set_default(ctx).root_path, wait_for_unlock=wait_for_unlock))
