...
def create_cactus_directory(
    ...
    """
    This function creates a new cactus directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from cactus.cmds.init_funcs import cactus_init

    if not cactus_root.is_dir() or not Path(cactus_root / "config" / "config.yaml").exists():
        # create cactus directories & load config
        cactus_init(cactus_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(cactus_root, "config.yaml")
        ...
        if not docker_mode:  # We want predictable ports for our docker image.
            ...
            config["network_overrides"]["config"]["simulator0"]["default_full_node_port"] = 311444 + port_offset
            # wallet
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `cactus keys show`.
    """
    ...
    print("Farmer public key (m/12381/11444/0/0):", master_sk_to_farmer_sk(sk).get_g1())
    print("Pool public key (m/12381/11444/1/0):", master_sk_to_pool_sk(sk).get_g1())
    ...
    print("First wallet secret key (m/12381/11444/2/0):", master_sk_to_wallet_sk(sk, uint32(0)))
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from cactus.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["CACTUS_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create cactus directory & get config.
    print("Creating cactus directory & config...")
    config = create_cactus_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "cactus")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your CACTUS_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from cactus.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path, fingerprint) as (
        ...
        if node_client is not None:
            ...
            # chain status ( basically cactus show -s)
            ...
            print(
                ...
                f"with a balance of: "
                f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['cactus']} TCAC."
            )
            if show_addresses:
                print("All Addresses: ")
