...
from cactus.cmds.param_types import CliAddress, CliAmount
...
from cactus.wallet.conditions import ConditionValidTimes, CreateCoinAnnouncement, CreatePuzzleAnnouncement
    ...
    *,
    ...
    async with get_wallet_client(root_path, wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if not override and check_unusual_transaction(final_amount, fee):
            print(
                f"A transaction of amount {final_amount / units['cactus']} and fee {fee} is unusual.\n"
            ...
            for item in [*offers, *requests]:
                                    ...
                                    royalty_assets.append(
                                        RoyaltyAsset(
                                            ...
                                            royalty_address=encode_puzzle_hash(
                                                info.royalty_puzzle_hash, AddressType.CAC.hrp(config)
                                            ),
                ...
                with filepath.open(mode="w") as file:
                    ...
                    if res.offer is not None:
                        ...
                        print(
                            f"Use cactus wallet get_offers --id "
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: dict[str, str], has_fee: bool = False, network_cac: str = "CAC"
) -> None:
    ...
    async with get_wallet_client(root_path, wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
            ...
            if royalty_asset_id.hex() in offered:
                ...
                royalty_assets.append(
                    RoyaltyAsset(
                        ...
                        royalty_address=encode_puzzle_hash(address, AddressType.CAC.hrp(config)),
        ...
        if not examine_only:
            ...
            if push:
                ...
                print(
                    f"Use cactus wallet get_offers --id {res.trade_record.trade_id} -f {fingerprint} to view its status"
                )
...
async def send_notification(
    ...
    async with get_wallet_client(root_path, wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        amount: uint64 = cli_amount.convert_amount(units["cactus"])
        ...
        if push:
            ...
            print(
                "To get status, use command: cactus wallet get_transaction"
    ...
    async with get_wallet_client(root_path, wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        res = await wallet_client.vc_mint(
            VCMint(
                ...
                target_address=target_address.validate_address_type(AddressType.CAC) if target_address else None,
                fee=fee,
                push=push,
            ),
            CMDTXConfigLoader().to_tx_config(units["cactus"], config, fingerprint),
