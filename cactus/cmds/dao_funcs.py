...
from cactus.cmds.cmds_util import CMDTXConfigLoader, get_wallet_client, transaction_status_msg, transaction_submitted_msg
from cactus.cmds.units import units
from cactus.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type
...
from cactus.wallet.util.tx_config import DEFAULT_COIN_SELECTION_CONFIG
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        res = await wallet_client.create_new_dao_wallet(
            ...
            tx_config=CMDTXConfigLoader.from_json_dict({"reuse_puzhash": True}).to_tx_config(
                units["cactus"], config, fingerprint
            ),
            dao_rules=None,
            amount_of_cats=None,
...
async def create_dao_wallet(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    proposal_minimum = uint64(int(Decimal(args["proposal_minimum_amount"]) * units["cactus"]))
    ...
    final_fee: uint64 = uint64(int(fee * units["cactus"]))

    fee_for_cat = Decimal(args["fee_for_cat"])
    final_fee_for_cat: uint64 = uint64(int(fee_for_cat * units["cactus"]))
...
async def vote_on_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    fee = args["fee"]
    final_fee: uint64 = uint64(int(Decimal(fee) * units["cactus"]))
    proposal_id = args["proposal_id"]
