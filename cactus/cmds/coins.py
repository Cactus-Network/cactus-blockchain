...
from cactus.cmds.cmd_classes import (
    cactus_command,
    option,
)
...
from cactus.cmds.param_types import AmountParamType, Bytes32ParamType, CliAmount
...
class ListCMD:
    ...
    async def run(self) -> None:
        async with self.rpc_info.wallet_rpc() as wallet_rpc:
            from cactus.cmds.coin_funcs import async_list
...
class CombineCMD(TransactionEndpoint):
    ...
    async def run(self) -> list[TransactionRecord]:
        async with self.rpc_info.wallet_rpc() as wallet_rpc:
            from cactus.cmds.coin_funcs import async_combine
...
class SplitCMD(TransactionEndpoint):
    ...
    async def run(self) -> list[TransactionRecord]:
        async with self.rpc_info.wallet_rpc() as wallet_rpc:
            from cactus.cmds.coin_funcs import async_split
