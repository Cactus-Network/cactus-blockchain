...
from cactus.cmds.cmds_util import CMDCoinSelectionConfigLoader, CMDTXConfigLoader, cli_confirm
...
from cactus.wallet.wallet_request_types import CombineCoins, GetCoinRecordsByNames, GetSpendableCoins, SplitCoins
    ...
    *,
    ...
    if push:
        ...
        for tx in resp.transactions:
            ...
            print(
                f"To get status, use command: cactus wallet get_transaction -f {client_info.fingerprint} -tx 0x{tx.name}"
            )
...
async def async_split(
    *,
    ...
    dust_threshold = client_info.config.get("cac_spam_amount", 1000000)  # min amount per coin in mojo
