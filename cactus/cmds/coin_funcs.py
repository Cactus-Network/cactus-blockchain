...
from cactus.cmds.cmds_util import CMDCoinSelectionConfigLoader, CMDTXConfigLoader, cli_confirm, get_wallet_client
from cactus.cmds.units import units
from cactus.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type, print_balance
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        if not await wallet_client.get_synced():
            print("Wallet not synced. Please wait.")
            return
        is_cac: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly combining Cactus
        ...
        if final_target_coin_amount != 0:  # if we have a set target, just use standard coin selection.
            removals: List[Coin] = await wallet_client.select_coins(
                amount=(final_target_coin_amount + final_fee) if is_cac else final_target_coin_amount,
                wallet_id=wallet_id,
        ...
        if is_cac and total_amount - final_fee <= 0:
        ...
        additions = [{"amount": (total_amount - final_fee) if is_cac else total_amount, "puzzle_hash": target_ph}]
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        if not await wallet_client.get_synced():
            print("Wallet not synced. Please wait.")
            return
        is_cac: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly spitting Cactus
