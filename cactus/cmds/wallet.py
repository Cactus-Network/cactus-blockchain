...
from cactus.cmds import options
from cactus.cmds.check_wallet_db import help_text as check_help_text
from cactus.cmds.cmd_classes import CactusCliContext, get_cactus_command_metadata
from cactus.cmds.cmds_util import timelock_args, tx_out_cmd
from cactus.cmds.coins import coins_cmd
...
from cactus.cmds.signer import PushTransactionsCMD, signer_cmd
...
wallet_cmd.add_command(get_cactus_command_metadata(PushTransactionsCMD).command)
...
def get_transaction_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_transaction
...
def get_transactions_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_transactions
...
@wallet_cmd.command("send", help="Send cactus or other assets to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
def send_cmd(
    ...
    from cactus.cmds.wallet_funcs import send
...
def show_cmd(ctx: click.Context, wallet_rpc_port: int | None, fingerprint: int, wallet_type: str | None) -> None:
    from cactus.cmds.wallet_funcs import print_balances
...
def get_address_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_address

    asyncio.run(get_address(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, id, new_address))
...
def clawback(
    ...
    from cactus.cmds.wallet_funcs import spend_clawback
...
def delete_unconfirmed_transactions_cmd(
    ...
    from cactus.cmds.wallet_funcs import delete_unconfirmed_transactions

    asyncio.run(
        delete_unconfirmed_transactions(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, id)
    )
...
def get_derivation_index_cmd(ctx: click.Context, wallet_rpc_port: int | None, fingerprint: int) -> None:
    from cactus.cmds.wallet_funcs import get_derivation_index

    asyncio.run(get_derivation_index(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint))
...
def address_sign_message(
    ...
    from cactus.cmds.wallet_funcs import sign_message
...
def update_derivation_index_cmd(ctx: click.Context, wallet_rpc_port: int | None, fingerprint: int, index: int) -> None:
    from cactus.cmds.wallet_funcs import update_derivation_index

    asyncio.run(update_derivation_index(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, index))
...
def add_token_cmd(
    ...
    from cactus.cmds.wallet_funcs import add_token

    asyncio.run(
        add_token(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, asset_id, token_name)
    )
...
def make_offer_cmd(
    ...
    from cactus.cmds.wallet_funcs import make_offer
...
def get_offers_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_offers
...
def take_offer_cmd(
    ...
    from cactus.cmds.wallet_funcs import take_offer
...
def cancel_offer_cmd(
    ...
    from cactus.cmds.wallet_funcs import cancel_offer
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck Cactus Wallet DBs"""

    from cactus.cmds.check_wallet_db import scan

    asyncio.run(scan(CactusCliContext.set_default(ctx).root_path, db_path, verbose=verbose))
...
def did_create_wallet_cmd(
    ...
    from cactus.cmds.wallet_funcs import create_did_wallet
...
def did_wallet_name_cmd(ctx: click.Context, wallet_rpc_port: int | None, fingerprint: int, id: int, name: str) -> None:
    from cactus.cmds.wallet_funcs import did_set_wallet_name

    asyncio.run(did_set_wallet_name(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, id, name))
...
def did_get_did_cmd(ctx: click.Context, wallet_rpc_port: int | None, fingerprint: int, id: int) -> None:
    from cactus.cmds.wallet_funcs import get_did

    asyncio.run(get_did(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, id))
...
def did_get_details_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_did_info

    asyncio.run(get_did_info(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, coin_id, latest))
...
def did_update_metadata_cmd(
    ...
    from cactus.cmds.wallet_funcs import update_did_metadata
...
def did_find_lost_cmd(
    ...
    from cactus.cmds.wallet_funcs import find_lost_did
...
def did_message_spend_cmd(
    ...
    from cactus.cmds.wallet_funcs import did_message_spend
...
def did_transfer_did(
    ...
    from cactus.cmds.wallet_funcs import transfer_did
...
def nft_wallet_create_cmd(
    ...
    from cactus.cmds.wallet_funcs import create_nft_wallet

    asyncio.run(
        create_nft_wallet(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, did_id, name)
    )
...
def nft_mint_cmd(
    ...
    from cactus.cmds.wallet_funcs import mint_nft
...
def nft_add_uri_cmd(
    ...
    from cactus.cmds.wallet_funcs import add_uri_to_nft
...
def nft_transfer_cmd(
    ...
    from cactus.cmds.wallet_funcs import transfer_nft
...
def nft_list_cmd(
    ...
    from cactus.cmds.wallet_funcs import list_nfts

    asyncio.run(
        list_nfts(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, id, num, start_index)
    )
...
def nft_set_did_cmd(
    ...
    from cactus.cmds.wallet_funcs import set_nft_did
...
def nft_get_info_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_nft_info

    asyncio.run(get_nft_info(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, nft_coin_id))
...
def send_notification_cmd(
    ...
    from cactus.cmds.wallet_funcs import send_notification
...
def get_notifications_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_notifications

    asyncio.run(
        get_notifications(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, id, start, end)
    )
...
def delete_notifications_cmd(
    ...
    from cactus.cmds.wallet_funcs import delete_notifications

    asyncio.run(delete_notifications(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, id, all))
...
def mint_vc_cmd(
    ...
    from cactus.cmds.wallet_funcs import mint_vc
...
def get_vcs_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_vcs

    asyncio.run(get_vcs(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, start, count))
...
def spend_vc_cmd(
    ...
    from cactus.cmds.wallet_funcs import spend_vc
...
def add_proof_reveal_cmd(
    ...
    from cactus.cmds.wallet_funcs import add_proof_reveal

    asyncio.run(
        add_proof_reveal(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, proof, root_only)
    )
...
def get_proofs_for_root_cmd(
    ...
    from cactus.cmds.wallet_funcs import get_proofs_for_root

    asyncio.run(
        get_proofs_for_root(CactusCliContext.set_default(ctx).root_path, wallet_rpc_port, fingerprint, proof_hash)
    )
...
def revoke_vc_cmd(
    ...
    from cactus.cmds.wallet_funcs import revoke_vc
...
def approve_r_cats_cmd(
    ...
    from cactus.cmds.wallet_funcs import approve_r_cats
