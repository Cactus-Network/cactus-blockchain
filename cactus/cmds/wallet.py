...
@wallet_cmd.command("send", help="Send cactus to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in CAC", default="0", show_default=True
)
@click.option(
    "--force",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in CAC", default="0", show_default=True
)
@click.option(
    "--reuse",
    help="Reuse existing address for the offer.",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in CAC", default="0", show_default=True
)
...
@click.option("-m", "--fee", help="The fee for the transaction, in CAC", type=str)
...
@click.option("-m", "--fee", help="Blockchain fee for mint transaction, in CAC", type=str, required=False, default="0")
def mint_vc_cmd(
    wallet_rpc_port: Optional[int],
    fingerprint: int,
    did: str,
...
@click.option(
    "-m", "--fee", help="Blockchain fee for update transaction, in CAC", type=str, required=False, default="0"
)
@click.option(
...
@click.option(
    "-m", "--fee", help="Blockchain fee for revocation transaction, in CAC", type=str, required=False, default="0"
)
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
...
@click.option(
    "-m", "--fee", type=str, default=0, show_default=True, help="Blockchain fee for approval transaction, in CAC"
)
