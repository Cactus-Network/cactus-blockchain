...
def challenges_cmd(ctx: click.Context, farmer_rpc_port: int | None, limit: int) -> None:
    ...
    from cactus.cmds.farm_funcs import challenges

    asyncio.run(challenges(CactusCliContext.set_default(ctx).root_path, farmer_rpc_port, limit))
...
def connect_solver_cmd(
    ...
    from cactus.cmds.farm_funcs import solver_connect

    asyncio.run(solver_connect(CactusCliContext.set_default(ctx).root_path, farmer_rpc_port, solver_address))
