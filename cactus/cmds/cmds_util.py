...
from cactus.cmds.param_types import AmountParamType, Bytes32ParamType, CliAmount, cli_amount_none
from cactus.consensus.default_constants import DEFAULT_CONSTANTS
from cactus.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate
...
from cactus.rpc.rpc_client import ResponseFailureError, RpcClient
...
from cactus.util.errors import CliRpcConnectionError, InvalidPathError
...
from cactus.wallet.util.tx_config import CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig, TXConfigLoader
from cactus.wallet.wallet_request_types import LogIn
...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'cactus wallet get_transaction -f {fingerprint} -tx 0x{tx_id.hex()}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                raise CliRpcConnectionError("No keys loaded. Run 'cactus keys generate' or import a key")
            ...
            click.option(
                ...
                click.option(
                    ...
                    click.option(
                        ...
                        multiple=True,
                        type=AmountParamType(),
                        help="Exclude any coins with this CAC or CAT amount from being included.",
                    )(
                        click.option(
                            "--primary-coin",
                            "primary_coin",
