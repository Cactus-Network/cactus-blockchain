...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if "cac_target_address" not in config["farmer"]:
        print(
            f"Setting the cac destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False, testnet: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if (
                cactus_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet) == 0
        ...
        return cactus_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet)
