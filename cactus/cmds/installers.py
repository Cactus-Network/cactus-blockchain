...
from cactus.daemon.server import executable_for_service
...
def check_plotter(plotter: List[str], expected_output: bytes, specify_tmp: bool = True) -> None:
    with tempfile.TemporaryDirectory() as path:
        ...
        process = subprocess.Popen(
            [executable_for_service("cactus"), "plotters", *plotter, *tmp_dir, "--final_dir", path],
...
@click.option("--expected-cactus-version", "expected_cactus_version_str", required=True)
@click.option("--require-madmax/--require-no-madmax", "require_madmax", default=True)
def test_command(expected_cactus_version_str: str, require_madmax: bool) -> None:
    print("testing installed executables")
    expected_cactus_version = packaging.version.Version(expected_cactus_version_str)

    args = [executable_for_service("cactus"), "version"]
    print(f"launching: {args}")
    cactus_version_process = subprocess.run(
        args,
        capture_output=True,
        encoding="utf-8",
        timeout=adjusted_timeout(30),
    )
    assert cactus_version_process.returncode == 0
    assert cactus_version_process.stderr == ""

    cactus_version = packaging.version.Version(cactus_version_process.stdout)
    print(cactus_version)
    assert cactus_version == expected_cactus_version, f"{cactus_version} != {expected_cactus_version}"

    args = [executable_for_service("cactus"), "plotters", "version"]
    print(f"launching: {args}")
    ...
    # check_plotter(plotter=["madmax"], expected_output=b"Multi-threaded pipelined Cactus")
    # check_plotter(plotter=["bladebit", "diskplot", "--compress", "0"], expected_output=b"\nBladebit Cactus Plotter")
    ...
    args = [executable_for_service("cactus"), "init"]
    print(f"launching: {args}")
    subprocess.run(
        args,
        check=True,
