...
from cactus.farmer.farmer import Farmer, increment_pool_stats, strip_old_entries
from cactus.harvester.harvester_api import HarvesterAPI
from cactus.protocols import farmer_protocol, harvester_protocol
from cactus.protocols.farmer_protocol import DeclareProofOfSpace, SignedValues
...
from cactus.protocols.solver_protocol import SolverInfo, SolverResponse
...
class FarmerAPI:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={
                                "User-Agent": f"Cactus Blockchain v.{__version__}",
                                "cactus-farmer-version": __version__,
                                "cactus-harvester-version": peer.version,
                            },
                        ) as resp:
                            if not resp.ok:
    ...
    @metadata.request(peer_required=True)
    async def partial_proofs(self, partial_proof_data: PartialProofsData, peer: WSCactusConnection) -> None:
        """
    ...
    @metadata.request()
    async def solution_response(self, response: SolverResponse, peer: WSCactusConnection) -> None:
        """
    ...
    async def new_signage_point(self, new_signage_point: farmer_protocol.NewSignagePoint) -> None:
        ...
        try:
            ...
            def old_harvesters(conn: WSCactusConnection) -> bool:
                return conn.protocol_version <= Version("0.0.36")

            def new_harvesters(conn: WSCactusConnection) -> bool:
                return conn.protocol_version > Version("0.0.36")
    ...
    @metadata.request(peer_required=True)
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: WSCactusConnection) -> None:
