...
from cactus.server.server import CactusServer, ssl_context_for_root
...
from cactus.util.ints import uint8, uint16, uint64
from cactus.util.keychain import Keychain
from cactus.util.logging import TimedDuplicateFilter
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        ...
        if "_target_address" not in self.config:
            self.config = config["farmer"]
        if "_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "_target_address" not in self.config or "_target_address" not in self.pool_config:
            log.debug("_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]) -> None:
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["_target_address"] = pool_target_encoded
