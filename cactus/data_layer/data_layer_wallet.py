...
from cactus.data_layer.data_layer_util import OfferStore, StoreProofs, leaf_hash
...
from cactus.types.blockchain_format.program import Program, run_with_cost
...
from cactus.wallet.singleton import SINGLETON_LAUNCHER_PUZZLE, SINGLETON_LAUNCHER_PUZZLE_HASH
...
class DataLayerWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSCactusConnection, coin_data: object | None) -> None:
    ...
    async def make_update_offer(
        ...
        for asset_id in offer_dict:
            if asset_id is None:
                raise ValueError("DataLayer update offers cannot include an CAC leg")
