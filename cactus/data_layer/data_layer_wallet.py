...
from cactus.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from cactus.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from cactus.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from cactus.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
...
from cactus.wallet.util.merkle_utils import _simplify_merkle_proof
from cactus.wallet.util.transaction_type import TransactionType
from cactus.wallet.util.tx_config import CoinSelectionConfig, TXConfig, TXConfigLoader
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSCactusConnection) -> CoinState:
    ...
    async def create_update_state_spend(
        ...
        if fee > 0:
            ...
            assert cactus_tx.spend_bundle is not None
            aggregate_bundle = SpendBundle.aggregate([dl_tx.spend_bundle, cactus_tx.spend_bundle])
            dl_tx = dataclasses.replace(dl_tx, spend_bundle=aggregate_bundle)
            cactus_tx = dataclasses.replace(cactus_tx, spend_bundle=None)
            txs: List[TransactionRecord] = [dl_tx, cactus_tx]
        else:
            txs = [dl_tx]
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, cactus_tx.spend_bundle])
                ),
                dataclasses.replace(cactus_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSCactusConnection, coin_data: Optional[object]) -> None:
