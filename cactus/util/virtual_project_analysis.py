...
# by default, all files are considered part of the "cactus-blockchain" project.

# To pull out a sub project, annotate its files with a comment (on the first
# line):
# Package: <name>

# if cactus-blockchain depends on this new sub-project, the sub-project may not
# depend back on cactus-blockchain.
...
class Annotation:
    ...
    def parse(cls, file_string: str) -> Annotation:
        ...
        if result is None:
            return cls("cactus-blockchain", False)
...
class CactusFile:
    ...
    @classmethod
    def parse(cls, file_path: Path) -> CactusFile:
        # everything under cactus/_tests belong to the "tests" subproject. It
        # (obviously) depends on everything, but no production code is allowed
        # to depend back on the tests.
        if list(file_path.parts[0:2]) == ["cactus", "_tests"]:
...
def build_dependency_graph(dir_params: DirectoryParameters) -> dict[Path, list[Path]]:
    ...
    for cactus_file in dir_params.gather_non_empty_python_files():
        dependency_graph[cactus_file.path] = []
        with open(cactus_file.path, encoding="utf-8", errors="ignore") as f:
            filestring = f.read()
            tree = ast.parse(filestring, filename=cactus_file.path)
                    ...
                    if node.module is not None and node.module.startswith(dir_params.dir_path.stem):
                        ...
                        for path_to_search in paths_to_search:
                            if os.path.exists(path_to_search):
                                dependency_graph[cactus_file.path].append(Path(path_to_search))
                    ...
                    for alias in node.names:
                        if alias.name.startswith(dir_params.dir_path.stem):
                            ...
                            if os.path.exists(imported_path):
                                dependency_graph[cactus_file.path].append(Path(imported_path))
...
def build_virtual_dependency_graph(
    ...
    for file, imports in graph.items():
        file_path = Path(file)
        root_file = CactusFile.parse(file_path)
        if root_file.annotations is None:
            continue
        root = root_file.annotations.package
        virtual_graph.setdefault(root, [])

        dependency_files = [CactusFile.parse(Path(imp)) for imp in imports]
...
def find_cycles(
    ...
    for dependent in sorted(graph):
        ...
        dependent_file = CactusFile.parse(dependent)
        ...
        for provider in sorted(graph[dependent]):
            if provider in excluded_paths:
                continue
            provider_file = CactusFile.parse(provider)
            if (
...
class DirectoryParameters:
    ...
    def gather_non_empty_python_files(self) -> list[CactusFile]:
        """
            ...
            for file in files:
                ...
                if file_path.suffix == ".py" and file_path not in self.excluded_paths:
                    ...
                    if os.path.getsize(file_path) > 0:
                        python_files.append(CactusFile.parse(file_path))
...
@click.command("find_missing_annotations", short_help="Search a directory for cactus files without annotations")
@config
