...
from cactus.wallet.puzzle_drivers import Solver
...
from cactus.wallet.vc_wallet.cr_cat_drivers import CRCAT, CRCATSpend, ProofsChecker, construct_pending_approval_state
from cactus.wallet.vc_wallet.vc_drivers import VerifiedCredential
from cactus.wallet.vc_wallet.vc_store import VCProofs, VCRecord, VCStore
...
if TYPE_CHECKING:
    from cactus.wallet.wallet_state_manager import WalletStateManager  # pragma: no cover
...
class VCWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSCactusConnection, coin_data: Optional[Streamable]
    ) -> None:
        """
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if fee > 0:
            ...
            cactus_tx = await self.wallet_state_manager.main_wallet.create_tandem_cac_tx(
        ...
        if cactus_tx is not None and cactus_tx.spend_bundle is not None:
            spend_bundles.append(cactus_tx.spend_bundle)
            tx_list.append(dataclasses.replace(cactus_tx, spend_bundle=None))
    ...
    async def revoke_vc(
        ...
        if fee > 0:
            cactus_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.create_tandem_cac_tx(
                fee, tx_config, vc_announcement
            )
            assert did_tx.spend_bundle is not None
            assert cactus_tx.spend_bundle is not None
            new_bundle = SpendBundle.aggregate([cactus_tx.spend_bundle, did_tx.spend_bundle])
