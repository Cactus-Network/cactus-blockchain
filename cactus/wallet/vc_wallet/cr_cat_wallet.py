...
from cactus.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from cactus.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
from cactus.wallet.cat_wallet.cat_wallet import CATWallet
from cactus.wallet.coin_selection import select_coins
from cactus.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from cactus.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSCactusConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_cactus_transaction:
                    if fee > regular_cactus_to_claim:
                        ...
                        cactus_tx, cac_announcement = await self.create_tandem_cac_tx(
                            fee,
                            uint64(regular_cactus_to_claim),
                            tx_config,
                        )
                        ...
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={cac_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [cactus_tx.spend_bundle]
            ...
            [*vc_txs, *([cactus_tx] if cactus_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(cactus_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if cactus_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, cactus_tx.spend_bundle])
        else:
            cactus_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(cactus_tx, spend_bundle=None),) if cactus_tx is not None else []),
        ]
