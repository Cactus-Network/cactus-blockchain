...
from cactus.data_layer.data_layer_wallet import DataLayerSummary, Mirror
from cactus.data_layer.singleton_record import SingletonRecord
from cactus.pools.pool_wallet_info import NewPoolWalletInitialTargetState, PoolWalletInfo
from cactus.types.blockchain_format.coin import coin_as_list
...
from cactus.util.streamable import Streamable, streamable, streamable_enum
...
from cactus.wallet.util.clvm_streamable import json_deserialize_with_clvm_streamable
from cactus.wallet.util.puzzle_decorator_type import PuzzleDecoratorType
from cactus.wallet.util.query_filter import TransactionTypeFilter
from cactus.wallet.util.tx_config import (
    CoinSelectionConfig,
...
class NFTMintBulk(TransactionEndpointRequest):
    ...
    cac_coins: list[Coin] | None = None
    cac_change_target: str | None = None
    ...
    def convert_to_proxy(self, proxy_type: type[_T_SendTransactionMultiProxy]) -> _T_SendTransactionMultiProxy:
        if proxy_type is CATSpend:
            if self.morph_bytes is not None:
                raise ValueError(
                    'Specified "morph_bytes" for a CAT-type wallet. Maybe you meant to specify an CAC wallet?'
                )
                ...
                raise ValueError(
                    ...
                    "Maybe you meant to specify an CAC wallet?"
                )
            ...
            if self.amount is not None:
                raise ValueError('Specified "amount" for an CAC wallet. Maybe you meant to specify a CAT-type wallet?')
            elif self.inner_address is not None:
                raise ValueError(
                    'Specified "inner_address" for an CAC wallet. Maybe you meant to specify a CAT-type wallet?'
                )
            elif self.memos is not None:
                raise ValueError('Specified "memos" for an CAC wallet. Maybe you meant to specify a CAT-type wallet?')
                ...
                raise ValueError('"additions" are required for CAC wallets.')
