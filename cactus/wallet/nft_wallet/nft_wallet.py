...
from cactus.wallet.conditions import Condition, UnknownCondition, parse_timelock_info
...
from cactus.wallet.util.tx_config import CoinSelectionConfig, TXConfig
...
class NFTWallet:
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            cactus_tx = await self.standard_wallet.create_tandem_cac_tx(
    ...
    async def mint_from_did(
        ...
        if cac_coins is None:
            cac_coins = await self.standard_wallet.select_coins(uint64(total_amount), tx_config.coin_selection_config)
        ...
        for cac_coin in cac_coins:
            ...
            if first:
                ...
                if len(cac_coins) > 1:
                    cac_announcement: Optional[Set[bytes]] = {message}
                else:
                    cac_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=[cac_payment],
                    fee=fee,
                    coin_announcements=cac_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(cac_coin.name(), message).name()
            ...
            cac_spends.append(CoinSpend(cac_coin, puzzle, solution))
        cac_spend = await self.wallet_state_manager.sign_transaction(cac_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, cac_spend, *eve_spends])
        ...
        for cac_coin in cac_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    ...
                    fee=fee,
                    coin_announcements=cac_announcement if len(cac_coins) > 1 else None,
