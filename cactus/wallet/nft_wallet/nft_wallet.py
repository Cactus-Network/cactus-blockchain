...
from cactus.wallet.trading.offer import (
    OFFER_MOD,
    OFFER_MOD_HASH,
    OFFER_MOD_OLD,
    OFFER_MOD_OLD_HASH,
...
from cactus.wallet.util.wallet_sync_utils import fetch_coin_spend
from cactus.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
from cactus.wallet.wallet import CHIP_0002_SIGN_MESSAGE_PREFIX, Wallet
...
class NFTWallet:
    ...
    async def mint_from_did(
        ...
        if cac_coins is None:
            cac_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        ...
        cac_primaries = [
            AmountWithPuzzlehash({"puzzlehash": cac_change_ph, "amount": change, "memos": [cac_change_ph]})
        ]
        ...
        for cac_coin in cac_coins:
            ...
            if first:
                ...
                message_list.append(
                    Coin(cac_coin.name(), cac_primaries[0]["puzzlehash"], cac_primaries[0]["amount"]).name()
                )
        ...
        cac_spend = await self.standard_wallet.sign_transaction(cac_spends)
    ...
    async def mint_from_cac(
        ...
        for cac_coin in cac_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=cac_primaries + primaries,
                    fee=fee,
