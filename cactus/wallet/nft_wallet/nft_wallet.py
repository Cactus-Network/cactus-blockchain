...
class NFTWallet:
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            cactus_tx = await self.standard_wallet.create_tandem__tx(
    ...
    async def mint_from_did(
        ...
        _coins: Optional[Set[Coin]] = None,
        _change_ph: Optional[bytes32] = None,
        ...
        :param _coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param _change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        if _coins is None:
            _coins = await self.standard_wallet.select_coins(uint64(total_amount))
        assert len(_coins) > 0
        ...
        spend_value = sum([coin.amount for coin in _coins])
        change: uint64 = uint64(spend_value - total_amount)
        _spends = []
        if _change_ph is None:
            _change_ph = await self.standard_wallet.get_new_puzzlehash()
        _primaries = [
            AmountWithPuzzlehash({"puzzlehash": _change_ph, "amount": change, "memos": [_change_ph]})
        ]

        first = True
        for _coin in _coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in _coins]
                message_list.append(
                    Coin(_coin.name(), _primaries[0]["puzzlehash"], _primaries[0]["amount"]).name()
                )
                message: bytes32 = std_hash(b"".join(message_list))

                if len(_coins) > 1:
                    _announcement: Optional[Set[bytes]] = {message}
                else:
                    _announcement = None
                ...
                solution: Program = self.standard_wallet.make_solution(
                    ...
                    fee=fee,
                    coin_announcements=_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(_coin.name(), message).name()
            ...
            _spends.append(CoinSpend(_coin, puzzle, solution))
        _spend = await self.standard_wallet.sign_transaction(_spends)
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, _spend, *eve_spends])
        return total_spend

    async def mint_from_(
        self,
        metadata_list: List[Dict[str, Any]],
        target_list: Optional[List[bytes32]] = [],
        mint_number_start: Optional[int] = 1,
        ...
        for _coin in _coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=_primaries + primaries,
                    fee=fee,
                    coin_announcements=_announcement if len(_coins) > 1 else None,
