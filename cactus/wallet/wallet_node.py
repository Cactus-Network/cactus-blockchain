...
from cactus.types.blockchain_format.coin import Coin, hash_coin_list
...
from cactus.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from cactus.util.path import mkdir, path_from_root
from cactus.wallet.block_record import HeaderBlockRecord
from cactus.wallet.derivation_record import DerivationRecord
from cactus.wallet.settings.settings_objects import BackupInitialized
from cactus.wallet.transaction_record import TransactionRecord
from cactus.wallet.util.backup_utils import open_backup_file
from cactus.wallet.util.wallet_types import WalletType
from cactus.wallet.wallet_action import WalletAction
from cactus.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSCactusConnection):
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSCactusConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSCactusConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSCactusConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
