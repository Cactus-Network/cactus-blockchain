...
from cactus.wallet.did_wallet.did_wallet_puzzles import uncurry_innerpuz
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSCactusConnection):
        ...
        if parent is None:
            ...
            parent_innerpuz = cactus.wallet.singleton.get_innerpuzzle_from_puzzle(req_puz_sol.puzzle.to_program())
            if parent_innerpuz:
    ...
    async def load_parent(self, did_info: DIDInfo):
        ...
        while True:
            ...
            if children_state.spent_height != children_state.created_height:
                ...
                parent_innerpuz = cactus.wallet.singleton.get_innerpuzzle_from_puzzle(
    ...
    def puzzle_for_pk(self, pubkey: G1Element) -> Program:
        if self.did_info.origin_coin is not None:
            ...
            return cactus.wallet.singleton.create_fullpuz(innerpuz, self.did_info.origin_coin.name())
        else:
            innerpuz = Program.to((8, 0))
            return cactus.wallet.singleton.create_fullpuz(innerpuz, bytes32([0] * 32))
    ...
    def puzzle_hash_for_pk(self, pubkey: G1Element) -> bytes32:
        ...
        return cactus.wallet.singleton.create_fullpuz_hash(innerpuz_hash, origin_coin_name)
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0), reuse_puzhash: Optional[bool] = None):
        ...
        full_puzzle: Program = cactus.wallet.singleton.create_fullpuz(
            innerpuz,
            self.did_info.origin_coin.name(),
        )
        ...
        new_full_puzzle: Program = cactus.wallet.singleton.create_fullpuz(
            new_inner_puzzle,
            self.did_info.origin_coin.name(),
        )
    ...
    async def generate_new_decentralised_id(self, amount: uint64, fee: uint64 = uint64(0)) -> Optional[SpendBundle]:
        ...
        genesis_launcher_puz = cactus.wallet.singleton.LAUNCHER_PUZZLE
        ...
        did_full_puz = cactus.wallet.singleton.create_fullpuz(did_inner, launcher_coin.name())
