...
from cactus.data_layer.data_layer_util import DLProof, VerifyProofResponse, dl_verify_proof
from cactus.data_layer.data_layer_wallet import Mirror
...
from cactus.rpc.util import ALL_TRANSLATION_LAYERS, RpcEndpoint, marshal
...
from cactus.util.streamable import UInt32Range
...
from cactus.wallet.util.clvm_streamable import json_serialize_with_clvm_streamable
from cactus.wallet.util.compute_memos import compute_memos
from cactus.wallet.util.query_filter import HashFilter
from cactus.wallet.util.signing import sign_message, verify_signature
...
from cactus.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
from cactus.wallet.wallet_coin_record import WalletCoinRecord, WalletCoinRecordMetadataParsingError
...
class WalletRpcApi:
                    ...
                    Addition(
                        ...
                        puzzle_hash=decode_puzzle_hash(
                            ensure_valid_address(
                                request.address, allowed_types={AddressType.CAC}, config=self.service.config
                            )
                        ),
                        memos=request.memos,
                    )
    ...
    async def did_get_info(self, request: DIDGetInfo) -> DIDGetInfoResponse:
        ...
        return DIDGetInfoResponse(
            ...
            p2_address=encode_puzzle_hash(search_results.p2_puzzle_hash, AddressType.CAC.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        if request.cac_change_target is not None:
            if request.cac_change_target.startswith(AddressType.CAC.hrp(self.service.config)):
                cac_change_ph = decode_puzzle_hash(request.cac_change_target)
            else:
                cac_change_ph = bytes32.from_hexstr(request.cac_change_target)
        else:
            cac_change_ph = None
        ...
        if request.mint_from_did:
            await nft_wallet.mint_from_did(
                ...
                cac_coins=set(request.cac_coins) if request.cac_coins is not None else None,
