...
from cactus.protocols.wallet_protocol import CoinState, RequestBlockHeader, RespondBlockHeader
...
from cactus.wallet import singleton
from cactus.wallet.cat_wallet.cat_utils import CAT_MOD, SpendableCAT, construct_cat_puzzle
from cactus.wallet.cat_wallet.cat_utils import get_innerpuzzle_from_puzzle as get_innerpuzzle_from_cat_puzzle
from cactus.wallet.cat_wallet.cat_utils import unsigned_spend_bundle_for_spendable_cats
...
from cactus.wallet.conditions import Condition, parse_timelock_info
from cactus.wallet.dao_wallet.dao_info import DAOInfo, DAORules, ProposalInfo, ProposalType
...
from cactus.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, TXConfig
...
class DAOWallet:
    ...
    It is not a user-facing wallet. A user cannot spend or receive CAC though a wallet of this type.
    ...
    # if asset_id == None: then we get normal CAC
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSCactusConnection, coin_data: Optional[Any]) -> None:
        """
        ...
        try:
            ...
            if matched_funding_puz:
                # funding coin
                cac_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
                if coin.puzzle_hash == cac_funds_puzhash:
                    asset_id = None
                else:
    ...
    async def resync_treasury_state(self) -> None:
        ...
        parent_inner_puz = cactus.wallet.singleton.get_inner_puzzle_from_singleton(
        ...
        # get existing cac funds for treasury
        cac_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
        await self.wallet_state_manager.add_interested_puzzle_hashes([cac_funds_puzhash], [self.id()])
    ...
    async def generate_proposal_vote_spend(
        ...
        if fee > 0:
            ...
            spend_bundle = unsigned_spend_bundle.aggregate([unsigned_spend_bundle, dao_cat_spend, cactus_tx.spend_bundle])
        else:
    ...
    async def create_proposal_close_spend(
        ...
        if proposal_state["passed"] and not self_destruct:
            ...
            if proposal_type == ProposalType.SPEND:
                ...
                if sum > 0:
                    cac_coins = await self.select_coins_for_asset_type(uint64(sum))
                    for cac_coin in cac_coins:
                        cac_parent_amount_list.append([cac_coin.parent_coin_info, cac_coin.amount])
                        solution = Program.to(
                            [
                                0,
                                treasury_inner_puzhash,
                        ...
                        coin_spends.append(CoinSpend(cac_coin, p2_singleton_puzzle, solution))
        ...
        if fee > 0:
            cactus_tx = await self.standard_wallet.create_tandem_cac_tx(fee, tx_config)
            assert cactus_tx.spend_bundle is not None
            full_spend = SpendBundle.aggregate([spend_bundle, cactus_tx.spend_bundle])
        else:
    ...
    async def free_coins_from_finished_proposals(
        ...
        if fee > 0:
            cactus_tx = await self.standard_wallet.create_tandem_cac_tx(fee, tx_config)
            assert cactus_tx.spend_bundle is not None
            full_spend = full_spend.aggregate([full_spend, cactus_tx.spend_bundle])
            ...
            if prop_info.proposal_id == proposal_id:
                ...
                if proposal_type == ProposalType.SPEND:
                    ...
                    dictionary: Dict[str, Any] = {
                        ...
                        "cac_conditions": cac_created_coins,
