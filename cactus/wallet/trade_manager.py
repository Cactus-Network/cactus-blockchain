...
from cactus.data_layer.data_layer_wallet import DataLayerSummary, DataLayerWallet
...
from cactus.types.blockchain_format.program import Program, run
...
from cactus.wallet.db_wallet.db_wallet_puzzles import ACS_MU_PH
...
from cactus.wallet.vc_wallet.cr_cat_drivers import ProofsChecker, construct_pending_approval_state
...
class TradeManager:
    ...
    async def coins_of_interest_farmed(
        self, coin_state: CoinState, fork_height: uint32 | None, peer: WSCactusConnection
    ) -> None:
        """
    ...
    async def respond_to_offer(
        ...
        async with action_scope.use() as interface:
            if interface.side_effects.get_unused_derivation_record_result is not None:
                ...
                # we might want to push. A ticket to this respect can be found [CACTUS-2984].
