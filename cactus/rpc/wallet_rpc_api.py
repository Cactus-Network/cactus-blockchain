...
class WalletRpcApi:
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("_target_address")
        pool_target = config["pool"].get("_target_address")
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        _coin_list = request.get("_coins", None)
        _coins = None
        if _coin_list:
            _coins = set([Coin.from_json_dict(_coin) for _coin in _coin_list])
        _change_target = request.get("_change_target", None)
        if _change_target is not None:
            if _change_target[:2] == "cac":
                _change_ph = decode_puzzle_hash(_change_target)
            else:
                _change_ph = bytes32(hexstr_to_bytes(_change_target))
        else:
            _change_ph = None
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
