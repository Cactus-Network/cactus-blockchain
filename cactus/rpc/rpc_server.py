...
from cactus.rpc.rpc_errors import structured_error_from_exception
from cactus.rpc.util import wrap_http_handler
from cactus.server.server import (
    CactusServer,
...
from cactus.util.cactus_logging import default_log_level, set_log_level
from cactus.util.config import str2bool
from cactus.util.json_util import dict_to_json_str
from cactus.util.network import WebServer, resolve
from cactus.util.task_referencer import create_referenced_task
from cactus.util.ws_message import (
    WsRpcMessage,
    create_payload,
    create_payload_dict,
    format_response,
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> CactusServer:
        ...
        # a property so as to be read only which allows CactusServer to satisfy
        # Optional[CactusServer]
        ...
    ...
    def get_connections(self, request_node_type: NodeType | None) -> list[dict[str, Any]]:
        ...
        cactus.rpc.rpc_server.default_get_connections()
        """
        ...
...
def default_get_connections(server: CactusServer, request_node_type: NodeType | None) -> list[dict[str, Any]]:
