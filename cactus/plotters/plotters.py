...
def call_plotters(root_path: Path, args):
    ...
    plotters = argparse.ArgumentParser("cactus plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, cactus_plotter_options, "chiapos", "Create a plot with the default cactus plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'cactus plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    ...
    deprecation_warning_bb2 = "[DEPRECATED] 'cactus plotters bladebit2' was integrated to 'cactus plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        show_plotters_version(cactus_root_path)
