...
from cactus.consensus.augmented_chain import AugmentedBlockchain
from cactus.consensus.block_body_validation import ForkInfo
from cactus.consensus.block_creation import unfinished_block_to_full_block_with_mmr
from cactus.consensus.block_height_map import BlockHeightMap
from cactus.consensus.blockchain import AddBlockResult, Blockchain, BlockchainMutexPriority, StateChangeSummary
from cactus.consensus.blockchain_interface import BlockchainInterface
from cactus.consensus.coin_store_protocol import CoinStoreProtocol
from cactus.consensus.condition_tools import pkm_pairs
from cactus.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from cactus.consensus.get_block_challenge import post_hard_fork2
from cactus.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from cactus.consensus.multiprocess_validation import PreValidationResult, pre_validate_block
from cactus.consensus.pot_iterations import calculate_sp_iters
from cactus.consensus.signage_point import SignagePoint
from cactus.full_node.block_store import BlockStore
from cactus.full_node.check_fork_next_block import check_fork_next_block
from cactus.full_node.coin_store import CoinStore
from cactus.full_node.full_node_api import FullNodeAPI
from cactus.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult, UnfinishedBlockEntry
...
from cactus.full_node.mempool import MempoolRemoveInfo
from cactus.full_node.mempool_manager import MempoolManager
from cactus.full_node.subscriptions import PeerSubscriptions, peers_for_spend_bundle
from cactus.full_node.sync_store import Peak, SyncStore
from cactus.full_node.tx_processing_queue import PeerWithTx, TransactionQueue, TransactionQueueEntry
...
from cactus.protocols.farmer_protocol import SignagePointSourceData, SPSubSlotSourceData, SPVDFSourceData
from cactus.protocols.full_node_protocol import RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
from cactus.protocols.outbound_message import Message, NodeType, make_msg
from cactus.protocols.protocol_message_types import ProtocolMessageTypes
from cactus.protocols.protocol_timing import CONSENSUS_ERROR_BAN_SECONDS
from cactus.protocols.shared_protocol import Capability
from cactus.protocols.wallet_protocol import CoinStateUpdate, RemovedMempoolItem
...
from cactus.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof, validate_vdf
from cactus.types.clvm_cost import QUOTE_BYTES, QUOTE_EXECUTION_COST
...
from cactus.types.validation_state import ValidationState
...
from cactus.util.cpu import available_logical_cores
...
from cactus.util.errors import ConsensusError, Err, TimestampError, ValidationError
from cactus.util.inline_executor import InlineExecutor
from cactus.util.limited_semaphore import LimitedSemaphore
from cactus.util.network import is_localhost
from cactus.util.path import path_from_root
from cactus.util.priority_thread_pool_executor import Executor, PriorityThreadPoolExecutor
from cactus.util.profiler import enable_profiler, mem_profile_task, profile_task
...
class PeakPostProcessingResult:
    ...
    signage_points: list[tuple[RespondSignagePoint, WSCactusConnection, EndOfSubSlotBundle | None]]
...
class FullNode:
    ...
    _server: CactusServer | None = None
    ...
    def set_server(self, server: CactusServer) -> None:
        self._server = server
    ...
    async def short_sync_batch(self, peer: WSCactusConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: WSCactusConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ) -> bool:
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSCactusConnection) -> None:
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: FullBlock | None = None, peer: WSCactusConnection | None = None
    ) -> None:
        """
    ...
    async def on_connect(self, connection: WSCactusConnection) -> None:
        """
    ...
    async def on_disconnect(self, connection: WSCactusConnection) -> None:
    ...
    async def request_validate_wp(
        ...
        weight_proof_peer: WSCactusConnection = random.choice(peers_with_peak)
        self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        # chain.
        blockchain = AugmentedBlockchain(self.blockchain)
        peers_with_peak: list[WSCactusConnection] = self.get_peers_with_peak(peak_hash)

        async def fetch_blocks(output_queue: asyncio.Queue[tuple[WSCactusConnection, list[FullBlock]] | None]) -> None:
            ...
            now = time.monotonic()
            new_peers_with_peak: list[tuple[WSCactusConnection, float]] = [(c, now) for c in peers_with_peak[:]]
        ...
        async def validate_blocks(
            input_queue: asyncio.Queue[tuple[WSCactusConnection, list[FullBlock]] | None],
            output_queue: asyncio.Queue[
                tuple[WSCactusConnection, ValidationState, list[Awaitable[PreValidationResult]], list[FullBlock]] | None
            ],
        ) -> None:
            nonlocal blockchain
            nonlocal fork_info
            ...
            try:
                while True:
                    res: tuple[WSCactusConnection, list[FullBlock]] | None = await input_queue.get()
                    if res is None:
        ...
        block_queue: asyncio.Queue[tuple[WSCactusConnection, list[FullBlock]] | None] = asyncio.Queue(maxsize=10)
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> list[WSCactusConnection]:
    ...
    async def add_block(
        self,
        block: FullBlock,
        peer: WSCactusConnection | None = None,
    ...
    async def add_unfinished_block(
        ...
        def old_clients(conn: WSCactusConnection) -> bool:
            # don't send this to peers with new clients
            return conn.protocol_version <= Version("0.0.35")

        def new_clients(conn: WSCactusConnection) -> bool:
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: WSCactusConnection | None = None
    ) -> Message | None:
    ...
    async def add_end_of_sub_slot(
        self, end_of_slot_bundle: EndOfSubSlotBundle, peer: WSCactusConnection
    ...
    async def broadcast_added_tx(self, mempool_item: MempoolItem, current_peer: WSCactusConnection | None = None) -> None:
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: WSCactusConnection) -> None:
        peak = self.blockchain.get_peak()
        if peak is None or peak.height - request.height < 5:
    ...
    async def add_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSCactusConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: WSCactusConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
    block_response: Any | None = await peer.call_api(
