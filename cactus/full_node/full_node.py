...
from cactus.consensus.blockchain import AddBlockResult, Blockchain, StateChangeSummary
...
from cactus.full_node.lock_queue import LockClient, LockQueue
...
class FullNode:
    ...
    async def _sync(self) -> None:
        ...
        try:
            ...
            peers_with_peak: List[WSCactusConnection] = [
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: WSCactusConnection = self.server.all_connections[peer]
