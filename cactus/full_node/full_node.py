...
from cactus_rs import AugSchemeMPL, BLSCache
...
from cactus.full_node.mempool_manager import MempoolManager, NewPeakItem
...
from cactus.full_node.tx_processing_queue import TransactionQueue
...
from cactus.protocols.wallet_protocol import CoinState, CoinStateUpdate, RemovedMempoolItem
...
class FullNode:
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[WSCactusConnection] = None
    ) -> None:
        """
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[WSCactusConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[WSCactusConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[WSCactusConnection, List[FullBlock]]]]
        ) -> None:
            ...
            block_rate_height = -1
            while True:
                res: Optional[Tuple[WSCactusConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[WSCactusConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[WSCactusConnection] = None
    ) -> Optional[Message]:
    ...
    async def add_transaction(
        self, transaction: SpendBundle, spend_name: bytes32, peer: Optional[WSCactusConnection] = None, test: bool = False
    ...
    async def broadcast_added_tx(
        self, mempool_item: MempoolItem, current_peer: Optional[WSCactusConnection] = None
    ) -> None:
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: WSCactusConnection) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
