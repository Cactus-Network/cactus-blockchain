...
from cactus.consensus.block_creation import calculate_infusion_point_total_iters, create_unfinished_block
from cactus.consensus.blockchain import BlockchainMutexPriority
from cactus.consensus.generator_tools import get_block_header
from cactus.consensus.get_block_generator import get_block_generator
...
from cactus.full_node.full_block_utils import get_height_and_tx_status_from_block, header_block_from_block
from cactus.full_node.full_node_store import SignagePointAddResult
from cactus.full_node.hard_fork_utils import get_flags
from cactus.full_node.tx_processing_queue import PeerWithTx, TransactionQueueEntry, TransactionQueueFull
from cactus.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from cactus.protocols.fee_estimate import FeeEstimate, FeeEstimateGroup, fee_rate_v2_to_v1
...
from cactus.protocols.protocol_timing import CONSENSUS_ERROR_BAN_SECONDS, RATE_LIMITER_BAN_SECONDS
...
from cactus.types.generator_types import BlockGenerator, NewBlockGenerator
...
from cactus.util.db_wrapper import SQLITE_MAX_VARIABLE_NUMBER
from cactus.util.errors import ConsensusError, Err
from cactus.util.hash import std_hash
from cactus.util.limited_semaphore import LimitedSemaphore, LimitedSemaphoreFullError
...
class FullNodeAPI:
    ...
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: WSCactusConnection) -> Message | None:
    ...
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: WSCactusConnection) -> Message | None:
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: WSCactusConnection
    ) -> Message | None:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: WSCactusConnection
    ) -> Message | None:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: WSCactusConnection
    ) -> Message | None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: WSCactusConnection
    ) -> Message | None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: WSCactusConnection
    ) -> Message | None:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: WSCactusConnection
    ) -> Message | None:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: WSCactusConnection
    ) -> Message | None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: WSCactusConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: WSCactusConnection
    ) -> Message | None:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def request_block_header(
        self, request: wallet_protocol.RequestBlockHeader, peer: WSCactusConnection
    ) -> Message | None:
    ...
    async def send_transaction(
        self, request: wallet_protocol.SendTransaction, peer: WSCactusConnection, *, test: bool = False
    ) -> Message | None:
    ...
    async def request_puzzle_solution(
        self, request: wallet_protocol.RequestPuzzleSolution, peer: WSCactusConnection
    ) -> Message | None:
    ...
    async def new_compact_vdf(
        self, request: full_node_protocol.NewCompactVDF, peer: WSCactusConnection, request_bytes: bytes = b""
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def request_compact_vdf(
        self, request: full_node_protocol.RequestCompactVDF, peer: WSCactusConnection
    ) -> Message | None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @metadata.request(peer_required=True)
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSCactusConnection) -> None:
    ...
    async def register_for_ph_updates(
        self, request: wallet_protocol.RegisterForPhUpdates, peer: WSCactusConnection
    ) -> Message:
        trusted = self.is_trusted(peer)
    ...
    async def register_for_coin_updates(
        self, request: wallet_protocol.RegisterForCoinUpdates, peer: WSCactusConnection
    ) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
    ...
    async def request_remove_puzzle_subscriptions(
        self, request: wallet_protocol.RequestRemovePuzzleSubscriptions, peer: WSCactusConnection
    ) -> Message:
    ...
    async def request_remove_coin_subscriptions(
        self, request: wallet_protocol.RequestRemoveCoinSubscriptions, peer: WSCactusConnection
    ) -> Message:
        peer_id = peer.peer_node_id
        subs = self.full_node.subscriptions
    ...
    async def request_puzzle_state(
        self, request: wallet_protocol.RequestPuzzleState, peer: WSCactusConnection
    ) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
        subs = self.full_node.subscriptions
    ...
    async def request_coin_state(self, request: wallet_protocol.RequestCoinState, peer: WSCactusConnection) -> Message:
        max_items = self.max_subscribe_response_items(peer)
        max_subscriptions = self.max_subscriptions(peer)
        subs = self.full_node.subscriptions
    ...
    async def mempool_updates_for_puzzle_hashes(
        self, peer: WSCactusConnection, puzzle_hashes: set[bytes32], include_hints: bool
    ) -> None:
    ...
    async def mempool_updates_for_coin_ids(self, peer: WSCactusConnection, coin_ids: set[bytes32]) -> None:
        if Capability.MEMPOOL_UPDATES not in peer.peer_capabilities:
            return
    ...
    def max_subscriptions(self, peer: WSCactusConnection) -> int:
    ...
    def max_subscribe_response_items(self, peer: WSCactusConnection) -> int:
        if self.is_trusted(peer):
    ...
    def is_trusted(self, peer: WSCactusConnection) -> bool:
