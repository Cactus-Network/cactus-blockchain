...
import { /* mojo_to_cactus_string, */ cactus_to_mojo } from '../../../util/cactus';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_cactus_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
  ...
  return (
    <Card title={<Trans>Balance</Trans>}>
      <BalanceCardSubSection
        ...
        tooltip={
          <Trans>
            This is the total amount of cactus in the blockchain at the current
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send cactus to coloured address. Please enter a cactus
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    const amountValue = Number.parseFloat(cactus_to_mojo(amount));
    const feeValue = Number.parseFloat(cactus_to_mojo(fee));
