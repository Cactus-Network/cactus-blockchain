...
export default function Block() {
  ...
  const poolReward = mojo_to_cactus(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_cactus(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const cactusFees = blockRecord.fees
    ? mojo_to_cactus(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.cactusexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.cactusexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
