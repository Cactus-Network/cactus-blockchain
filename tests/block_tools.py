...
from cactus.plotting.create_plots import create_plots
...
from cactus.consensus.cost_calculator import NPCResult, calculate_cost_of_program
...
from cactus.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from cactus.util.condition_tools import ConditionOpcode, conditions_by_opcode
...
class BlockTools:
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "cac")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("CACTUS_ROOT", "~/.cactus/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
