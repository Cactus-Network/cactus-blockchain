...
from cactus.cmds.keys import delete_all_cmd, generate_and_print_cmd, sign_cmd, verify_cmd
from cactus.util.config import load_config
from cactus.util.keychain import Keychain, KeyData, generate_mnemonic
from cactus.util.keyring_wrapper import DEFAULT_KEYS_ROOT_PATH, KeyringWrapper
...
class TestKeysCommands:
    def test_generate_with_new_config(self, tmp_path, empty_keyring):
        ...
        the correct _target_address entries.
        """
        ...
        # Verify that the config has the correct _target_address entries
        address_matches = re.findall(r"1[^\n]+", result.output)
        assert len(address_matches) > 1
        address = address_matches[0]

        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["_target_address"] == address
        assert config["pool"]["_target_address"] == address
    ...
    def test_generate_with_existing_config(self, tmp_path, empty_keyring):
        ...
        the original _target_address entries.
        """
        ...
        assert existing_config["farmer"]["_target_address"] == address
        assert existing_config["pool"]["_target_address"] == address
        ...
        # Verify that the config's _target_address entries have not changed
        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["_target_address"] == existing_config["farmer"]["_target_address"]
        assert config["pool"]["_target_address"] == existing_config["pool"]["_target_address"]
    ...
    def test_show_labels(self, empty_keyring, tmp_path):
        ...
        result = runner.invoke(cli, [*base_params, *cmd_params])
        assert result.output == "No keys are present in the keychain. Generate them with 'cactus keys generate'\n"
    ...
    def test_show_fingerprint(self, keyring_with_one_key, tmp_path):
        """
        Test that the `cactus keys show --fingerprint` command shows the correct key.
        """
    ...
    def test_show_json(self, keyring_with_one_key, tmp_path):
        """
        Test that the `cactus keys show --json` command shows the correct key.
        """
    ...
    def test_show_mnemonic_json(self, keyring_with_one_key, tmp_path):
        """
        Test that the `cactus keys show --show-mnemonic-seed --json` command shows the key's mnemonic seed.
        """
    ...
    def test_derive_search_wallet_address(self, tmp_path, keyring_with_one_key):
        ...
        result: Result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd",
            ],
        )
        ...
        assert (
            result.output.find(
                (
                    ...
                    "1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd (HD path: m/12381/8444/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        ...
        assert (
            result.output.find(
                (
                    "Found wallet address: "
                    "t1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs2v6lg7 (HD path: m/12381/8444/2/30)"
                )
            )
            != -1
        )
        ...
        assert (
            result.output.find(
                (
                    ...
                    "1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfs6wrfdq"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    ...
                    "1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8sumvfmt"
                )
            )
            != -1
        )
    ...
    def test_derive_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        ...
        assert (
            result.output.find(
                (
                    "Wallet address 50 (m/12381n/8444n/2n/50n): "
                    "t1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfshfylvn"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    "Wallet address 51 (m/12381n/8444n/2n/51n): "
                    "t1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8s3utl6c"
                )
            )
            != -1
        )
