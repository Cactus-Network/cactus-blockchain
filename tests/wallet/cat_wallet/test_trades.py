...
class TestCATTrades:
    ...
    async def test_cat_trades(
        ...
        MAKER_CACTUS_BALANCE = initial_maker_balance - 100
        TAKER_CACTUS_BALANCE = initial_taker_balance - 100
        ...
        if forwards_compat:
            ...
            success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
                cactus_for_cat, fee=uint64(1), reuse_puzhash=reuse_puzhash
            )
            await asyncio.sleep(1)
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        _to_cat_amount = uint64(100)

        async with wallet_node_maker.wallet_state_manager.lock:
            cat_wallet_maker: CATWallet = await CATWallet.create_new_cat_wallet(
                wallet_node_maker.wallet_state_manager, wallet_maker, {"identifier": "genesis_by_id"}, _to_cat_amount
            )
        ...
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, _to_cat_amount)
        await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, _to_cat_amount)
        maker_funds -= _to_cat_amount
