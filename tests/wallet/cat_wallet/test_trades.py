...
async def test_cat_trades(
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        cactus_for_cat, wallet_environments.tx_config, fee=uint64(1)
    )
    ...
    # cat_for_cactus
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        cat_for_cactus, wallet_environments.tx_config
    )
    assert error is None
    assert success is True
    assert trade_make is not None
...
class TestCATTrades:
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, cac_to_cat_amount)
        await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, cac_to_cat_amount)
        maker_funds -= cac_to_cat_amount
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cat_for_cactus, DEFAULT_TX_CONFIG)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cactus_for_cat, DEFAULT_TX_CONFIG)
        assert error is None
        assert success is True
        assert trade_make is not None
    ...
    async def test_aggregated_trade_state(self, wallets_prefarm):
        ...
        success, trade_make_1, error = await trade_manager_maker.create_offer_for_ids(cactus_for_cat, DEFAULT_TX_CONFIG)
        ...
        success, trade_make_2, error = await trade_manager_maker.create_offer_for_ids(cat_for_cactus, DEFAULT_TX_CONFIG)
        ...
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, cac_to_cat_amount - 1)
        await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, cac_to_cat_amount - 1)
