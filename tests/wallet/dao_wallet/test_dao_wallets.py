...
from cactus.simulator.setup_nodes import SimulatorsAndWallets, SimulatorsAndWalletsServices
...
async def test_dao_funding(
    ...
    funding_tx = await dao_wallet_0.create_add_funds_to_treasury_spend(cac_funds, DEFAULT_TX_CONFIG)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(cac_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal)
...
async def test_dao_proposals(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(
        cac_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal, fee=base_fee
    )
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
...
async def test_dao_complex_spends(
    ...
    try:
        ...
        cac_funds = uint64(10000000000)
        for _ in range(4):
            funding_tx = await client_0.dao_add_funds_to_treasury(dao_id_0, 1, uint64(cac_funds / 4), DEFAULT_TX_CONFIG)
            cat_funding_tx = await client_0.dao_add_funds_to_treasury(
...
async def test_dao_concurrency(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(
        cac_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal, uint64(1000)
    )
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
...
async def test_dao_votes(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(cac_proposal_inner, DEFAULT_TX_CONFIG, vote_1, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(cac_proposal_inner, DEFAULT_TX_CONFIG, vote_2, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    vote_5 = uint64(1)
    proposal_tx = await dao_wallet_0.generate_new_proposal(cac_proposal_inner, DEFAULT_TX_CONFIG, vote_5, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(cac_proposal_inner, DEFAULT_TX_CONFIG, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
...
async def test_dao_resync(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(cac_proposal_inner, DEFAULT_TX_CONFIG, uint64(10))
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallets_synced(wallet_nodes=[wallet_node_0, wallet_node_1], timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(cac_proposal_inner, DEFAULT_TX_CONFIG)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallets_synced(wallet_nodes=[wallet_node_0, wallet_node_1], timeout=30)
