...
async def test_send_transaction(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(await wallet_2.get_new_puzzlehash(), "t")
...
async def test_get_transactions(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    await client.send_transaction(
        1, uint64(1), encode_puzzle_hash(await wallet.get_new_puzzlehash(), "t")
    )  # Create a pending tx
    ...
    await client.send_transaction(1, uint64(1), encode_puzzle_hash(ph_by_addr, "t"))
    await client.farm_block(encode_puzzle_hash(ph_by_addr, "t"))
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)

    tx_for_address = await client.get_transactions(1, to_address=encode_puzzle_hash(ph_by_addr, "t"))
...
async def test_key_and_address_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    with lock_and_load_config(wallet_node.root_path, "config.yaml") as test_config:
        test_config["farmer"]["_target_address"] = encode_puzzle_hash(test_ph, "t")
        # set pool to second private key
        sk = await wallet_node.get_key_for_fingerprint(pks[1])
        test_ph = create_puzzlehash_for_pk(master_sk_to_wallet_sk(sk, uint32(0)).get_g1())
        test_config["pool"]["_target_address"] = encode_puzzle_hash(test_ph, "t")
...
# 1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
# $ cactus wallet sign_message -m $(echo -n 'hello world' | xxd -p)
# -a 1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
#
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            # -a 1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
        ...
        (
            ...
            # -a 1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh",
            },
            {"isValid": True},
        ),
        # Negative tests
        ...
        (
            # Valid signature but address doesn't match pubkey
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "1d0rekc2javy5gpruzmcnk4e4qq834jzlvxt5tcgl2ylt49t26gdsjen7t0",
            },
...
async def test_cat_spend_run_tail(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = await client.cat_spend(
        cat_wallet_id,
        amount=uint64(0),
        inner_address=encode_puzzle_hash(our_ph, "t"),
