...
async def test_nft_offer_with_fee(
    ...
    _request = 100
    maker_fee = uint64(10)
    offer_nft_for_ = {wallet_maker.id(): _request, nft_asset_id: -1}
    maker_unused_index = (
    ...
    if forwards_compat:
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            offer_nft_for_, driver_dict, fee=maker_fee, reuse_puzhash=reuse_puzhash
        )
    ...
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre + _request - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre - _request - taker_fee)
    ...
    _offered = 1000
    maker_fee = uint64(10)
    offer__for_nft = {wallet_maker.id(): -_offered, nft_to_buy_asset_id: 1}
    ...
    if forwards_compat:
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
            offer__for_nft, driver_dict_to_buy, fee=maker_fee
        )
        assert success is True
        assert error is None
        assert trade_make is not None
    ...
    if not forwards_compat:
        await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_maker, trade_make)
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_taker, trade_take)
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre - _offered - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre + _offered - taker_fee)
...
async def test_nft_offer_cancellations(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_nft_for_, driver_dict, fee=maker_fee
    )
    assert success is True
    assert error is None
    assert trade_make is not None
