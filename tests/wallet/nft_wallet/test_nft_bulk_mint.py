...
async def test_nft_mint_from_did(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    ...
    expected__bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected__bal)
...
async def test_nft_mint_from_did_rpc(
    ...
    try:
        ...
        _coins = await client.select_coins(amount=required_amount, wallet_id=wallet_maker.id())
        funding_coin = _coins[0]
        assert funding_coin.amount >= required_amount
        funding_coin_dict = _coins[0].to_json_dict()
        chunk = 5
        ...
        for i in range(0, n, chunk):
            ...
            resp: Dict[str, Any] = await client.nft_mint_bulk(
                ...
                mint_total=n,
                _coins=[next_coin.to_json_dict()],
                _change_target=funding_coin_dict["puzzle_hash"],
            ...
            spends.append(sb)
            _adds = [c for c in sb.additions() if c.puzzle_hash == funding_coin.puzzle_hash]
            assert len(_adds) == 1
            next_coin = _adds[0]
...
@pytest.mark.asyncio
async def test_nft_mint_from_did_multiple_(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    _coins_1 = await wallet_maker.select_coins(amount=10000)
    _coins_2 = await wallet_maker.select_coins(amount=10000, exclude=_coins_1)
    _coins = _coins_1.union(_coins_2)
    ...
    expected__bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_maker.get_confirmed_balance, expected__bal)
...
@pytest.mark.asyncio
async def test_nft_mint_from_(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    sb = await nft_wallet_maker.mint_from_(
        metadata_list, target_list=target_list, mint_number_start=1, mint_total=mint_total, fee=fee
    )
...
@pytest.mark.asyncio
async def test_nft_mint_from__rpc(
    two_wallet_nodes_services: SimulatorsAndWalletsServices, trusted: Any, self_hostname: str
) -> None:
    [full_node_service], wallet_services, bt = two_wallet_nodes_services
    full_node_api: FullNodeSimulator = full_node_service._api
...
@pytest.mark.asyncio
async def test_nft_mint_from__multiple_(self_hostname: str, two_wallet_nodes: Any, trusted: Any) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
