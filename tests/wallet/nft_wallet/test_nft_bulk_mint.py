...
async def test_nft_mint_from_did(
    ...
    metadata_list = [
        {
            "program": Program.to(
                [("u", ["https://www.cactus-network.network.net/img/branding/cactus-logo.svg"]), ("h", bytes32.random(seeded_random).hex())]
            ),
...
async def test_nft_mint_from_did_multiple_cac(
    ...
    cac_coins_1 = await wallet_maker.select_coins(amount=10000, coin_selection_config=DEFAULT_COIN_SELECTION_CONFIG)
    cac_coins_2 = await wallet_maker.select_coins(
        amount=10000,
        coin_selection_config=DEFAULT_COIN_SELECTION_CONFIG.override(excluded_coin_ids=[c.name() for c in cac_coins_1]),
    )
...
async def test_nft_mint_from_cac(
    ...
    tx_records = await nft_wallet_maker.mint_from_cac(
        metadata_list, DEFAULT_TX_CONFIG, target_list=target_list, mint_number_start=1, mint_total=mint_total, fee=fee
    )
    sb = tx_records[0].spend_bundle
    assert sb is not None
