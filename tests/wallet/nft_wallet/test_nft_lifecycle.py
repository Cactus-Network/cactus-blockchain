...
async def test_default_transfer_program(cost_logger: CostLogger) -> None:
    async with sim_and_client() as (sim, sim_client):
        ...
        _coin = (await sim_client.get_coin_records_by_puzzle_hash(ACS_PH, include_spent_coins=False))[0].coin
        ...
        for i in range(1, 3):
            for announcement_combo in itertools.combinations(
                [did_announcement_spend, _announcement_spend, cat_announcement_spend], i
            ):
        ...
        full_bundle = cost_logger.add_cost(
            ...
            SpendBundle(
                [ownership_spend, did_announcement_spend, _announcement_spend, cat_announcement_spend], G2Element()
            ),
        )
