...
- Bump `cactus_rs` to `0.16.0`

### Removed

- Python 3.8 support has been removed

## 2.4.4 Cactus blockchain 2024-10-15
...
- Display sync percentage in 'cactus show -s'
- Make 'cactus plotnft -f fingerprint -i wallet_id' output JSON
...
- Fix fee behavior with `cactus wallet coins combine`
...
## 2.4.3 Cactus blockchain 2024-08-21
...
- Remove ALERTS_URL and CACTUS_ALERTS_PUBKEY
...
## 2.4.2 Cactus blockchain 2024-07-10
...
## 2.4.1 Cactus blockchain 2024-06-25
...
## 2.4.0 Cactus blockchain 2024-06-20
...
- Bump `cactus_rs` to `0.9.0` and update G1Element handling
...
- Handle when cac_target_address in config doesn't decode correctly (fixes #16995)
...
- Added `--skip-keyring` option to `cactus start` and use in GUI (fixes #17848)

### Added

- Added Python 3.12 support
- Added new subscription and wallet sync protocol support (will be used by the wallet in future releases)
- Added Chip-25 Message Conditions support (https://github.com/Cactus-Network/chips/pull/98)
...
- Added `new_unfinished_block2` support in Cactus seeder
- Added Hint support for SpendSim

### Changed

- Transition `FullBlock`, `BlockRecord`, `CoinSpend`, and `HeaderBlock` to rust
- Move tests - all tests and infrastructure are now included in the cactus-blockchain package under `cactus/_tests`
...
## 2.2.1 Cactus blockchain 2024-03-4
...
## 2.2.0 Cactus blockchain 2024-02-28
...
- Fix sorted for dictionary keys of both bytes/cac
- Fixed an issue where `cactus wallet did transfer` command mistreats the type of `fee`
...
- move `tools/legacy_keyring.py` to `cactus/legacy/keyring.py`
...
- Update cactus_rs to 0.2.15 for AMD K10 architecture (fixes #16386)
...
- Use BLS from `cactus_rs` and stop using `blspy` wheel in cactus-blockchain
...
- bump `cactus_rs` to include bugfix for new `AGG_SIG\*` conditions in mempool mode
...
- Bump cactus_rs dependency to 0.2.9
...
- bump `cactus_rs` dependency to `0.2.0`
...
- Performance improvements in `subscribe_to_phs` using CoinState from cactus_rs
...
- Performance improvements in puzzle parsing by using rust parser (`cactus_rs`) for Program.from_bytes()
...
- Performance improvements by using `get_puzzle_and_solution_for_coin()` from `cactus_rs`
