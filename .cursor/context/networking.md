...
> Attach when touching `cactus/server/`, `cactus/protocols/`, connection handling,
...
| File                       | Lines | Role                                                 |
| -------------------------- | ----- | ---------------------------------------------------- |
| `server.py`                | ~900  | `CactusServer`: connection management, message routing |
| `ws_connection.py`         | ~790  | `WSCactusConnection`: single peer connection           |
...
| `cactus_policy.py`           | ~370  | Custom asyncio event loop policy, connection limits  |
...
## `WSCactusConnection` — Per-peer connection
...
- `peer_required=True` — handler receives `WSCactusConnection` as parameter
...
**Location**: `server/cactus_policy.py`

- Default `global_max_concurrent_connections = 250`
- `set_cactus_policy(connection_limit)` sets effective limit to `connection_limit + 100`
- Custom event loop policy (`CactusProactorEventLoop` on Windows,
