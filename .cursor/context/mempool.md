...
> Attach when touching `cactus/full_node/mempool*.py`,
...
- `cactus/types/internal_mempool_item.py` — `InternalMempoolItem` (signature separated)
- `cactus/types/mempool_inclusion_status.py` — `SUCCESS`, `FAILED`, `PENDING`
- `cactus/types/fee_rate.py` — `FeeRate`
- `cactus/types/clvm_cost.py` — `CLVMCost`, `QUOTE_BYTES`, `QUOTE_EXECUTION_COST`
...
| `MEMPOOL_MIN_FEE_INCREASE` | 10 000 000                | Min fee increase for replacement (0.00001 CAC)         |
| `MEMPOOL_ITEM_FEE_LIMIT`   | 2^50                      | Max fee per item (prevents SQLite int64 overflow)      |
| `nonzero_fee_minimum_fpc`  | 5                         | Min fee-per-cost to kick out others (~0.055 CAC/block) |
