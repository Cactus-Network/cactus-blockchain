...
> condition processing, or `cactus/types/blockchain_format/`.

## Key files

| File                                                 | Role                                                |
| ---------------------------------------------------- | --------------------------------------------------- |
| `cactus/consensus/condition_tools.py`                  | `pkm_pairs()` — extract AGG_SIG conditions          |
| `cactus/consensus/generator_tools.py`                  | `get_block_header()` — strip generator from block   |
| `cactus/consensus/get_block_generator.py`              | `get_block_generator()` — resolve generator refs    |
| `cactus/consensus/cost_calculator.py`                  | `NPCResult` — name/puzzle/conditions result         |
| `cactus/consensus/condition_costs.py`                  | Condition opcode costs                              |
| `cactus/types/blockchain_format/program.py`            | CLVM program wrapper                                |
| `cactus/types/blockchain_format/serialized_program.py` | Lazy deserialization                                |
| `cactus/types/blockchain_format/coin.py`               | `Coin(parent_id, puzzle_hash, amount)`              |
| `cactus/types/condition_opcodes.py`                    | All condition opcode values                         |
| `cactus/types/condition_with_args.py`                  | `ConditionWithArgs`                                 |
| `cactus/types/generator_types.py`                      | `BlockGenerator`, `NewBlockGenerator`               |
| `cactus/types/clvm_cost.py`                            | Cost constants                                      |
| `cactus/full_node/mempool_manager.py`                  | `pre_validate_spendbundle()`, `is_clvm_canonical()` |
| `cactus/wallet/conditions.py`                          | Condition construction for wallet spends            |
| `cactus/wallet/puzzles/`                               | CLVM puzzle source files                            |
...
**Location**: `cactus/types/condition_opcodes.py`
