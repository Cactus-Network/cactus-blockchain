...
if (-not (Test-Path env:CACTUS_SEMVER_VERSION)) {
  $env:CACTUS_SEMVER_VERSION = $env:CACTUS_INSTALLER_VERSION
  Write-Output "WARNING: No environment variable CACTUS_SEMVER_VERSION set. Using $env:CACTUS_INSTALLER_VERSION"
}

Write-Output "Cactus Version is: $env:CACTUS_INSTALLER_VERSION"
Write-Output "Cactus Semver Version is: $env:CACTUS_SEMVER_VERSION"
Write-Output "   ---"
...
jq --arg VER "$env:CACTUS_SEMVER_VERSION" '.version=$VER' package.json > temp.json
rm package.json
...
& "$NPM_PATH/electron-builder.ps1" build --win --x64 --config.productName="Cactus" --dir --config ../../../build_scripts/electron-builder.json
...
& "$NPM_PATH/electron-builder.ps1" build --win --x64 --config.productName="Cactus" --pd ".\dist\win-unpacked" --config ../../../build_scripts/electron-builder.json --publish never
Write-Output "   ---"
